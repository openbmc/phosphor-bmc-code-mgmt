...
{
    log<level::INFO>("Inside unsubscribeFromSystemdSignals");
    auto method = this->bus.new_method_call(SYSTEMD_BUSNAME, SYSTEMD_PATH,
    ...
    if (value == softwareServer::Activation::Activations::Activating)
    {
        ...
        else if (rwVolumeCreated == true && roVolumeCreated == true)
        {
            ...
            else
            {
                ...
                //Unsubscribe from systed signal if the apply time value
                //is OnReset
                if (Activation::checkApplyTimeImmediate() == false)
                {
...
void Activation::unitStateChange(sdbusplus::message::message& msg)
{
    if (softwareServer::Activation::activation() !=
        softwareServer::Activation::Activations::Activating)
    {
        log<level::INFO>("Exiting from unitStateChange");
        return;
    }
