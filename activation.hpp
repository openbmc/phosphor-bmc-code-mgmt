...
using VersionPurpose = sdbusplus::xyz::openbmc_project::Software::server::Version::VersionPurpose;
...
class Activation : public ActivationInherit, public Flash
{
  public:
    ...
    Activation(sdbusplus::bus::bus& bus, const std::string& path,
               ItemUpdater& parent, std::string& versionId, VersionPurpose purposeId,
               sdbusplus::xyz::openbmc_project::Software::server::Activation::
                   Activations activationStatus,
               AssociationList& assocs) :
        ActivationInherit(bus, path.c_str(), true),
        bus(bus), path(path), parent(parent), versionId(versionId), purposeId(purposeId),
        systemdSignals(
            bus,
