...
{
    debug("started async inotify watch for external bios version");
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    else
    {
        if (toolFlashrom)
        {
            success = co_await SPIDevice::
                writeSPIFlashFlashromHostOffGPIOSetDeviceBound(image,
                                                               image_size);
        }
        else
        {
    ...
    co_return (success && co_await SPIDevice::unbindSPIFlash());
}
...
std::string SPIDevice::readExternalVersion()
{
    std::string version{};
    try
    {
...
auto SPIDevice::processExternalVersionUpdate(std::string configFileName)
    -> sdbusplus::async::task<>
{
    ...
    softwareCurrent->setVersion(readExternalVersion());
