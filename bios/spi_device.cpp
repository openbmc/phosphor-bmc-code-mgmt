...
static std::optional<std::string> getSPIDevAddr(uint64_t spiControllerIndex){
	...
	const fs::path spi_path = "/sys/class/spi_master/spi" + std::to_string(spiControllerIndex);

    if (!fs::exists(spi_path)) {
	    error("SPI controller index not found at {PATH}", "PATH", spi_path.string());
	    return std::nullopt;
    }

    fs::path target = fs::read_symlink(spi_path);

    // The path looks like ../../devices/platform/ahb/1e630000.spi/spi_master/spi1
    // We want to extract e.g. '1e630000.spi'
