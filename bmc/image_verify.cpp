...
std::optional<PQAlgorithm> Signature::getPQAlgorithmFromManifest() const
{
    ...
    while (std::getline(efile, line))
    {
        ...
        PQAlgorithm algo;
        algo.hashFunc = value.substr(0, commaPos);
        algo.name = value.substr(commaPos + 1);

        // Convert hash function to lowercase for OpenSSL
        std::transform(algo.hashFunc.begin(), algo.hashFunc.end(),
                      algo.hashFunc.begin(), ::tolower);
    ...
    if (ret)
    {
        ...
        if (pqAlgo.has_value())
        {
            ...
            if (fs::exists(algoDir, ec2))
            {
                ...
                if (fs::exists(algoFullImageSig, ec2) && fs::exists(tmpFullFile, ec2))
                {
                    ret = verifyFile(tmpFullFile, algoFullImageSig, 
                                     algoPublicKeyFile, pqAlgo->hashFunc);
                    if (!ret)
                    {
            ...
            if (fs::exists(file, ec))
            {
                ...
                if (pqAlgo.has_value())
                {
                    ...
                    if (fs::exists(algoDir, ec))
                    {
                        ...
                        {
                            bool algoValid = verifyFile(file, algoSigFile, algoPublicKeyFile,
                                                        pqAlgo->hashFunc);
                            if (!algoValid)
                            {
                                error("Signature validation failed for optional image {IMAGE} with {ALGO}",
                                      "IMAGE", optionalImage, "ALGO", pqAlgo->name);
                                return false;
                            }
                        }
                    }
            ...
            if (valid)
            {
                ...
                if (valid)
                {
                   ...
                   if (pqAlgo.has_value())
                   {
                       ...
                       fs::path systemAlgoKeyPath = signedConfPath / keyType / 
                                                    pqAlgo->name / PUBLICKEY_FILE_NAME;
                       ...
                       // Verify MANIFEST and publickey with post-quantum algorithm
                       ...
                       valid = verifyFile(manifestFile, algoManifestSig,
                                          systemAlgoKeyPath, pqAlgo->hashFunc);
                       if (!valid)
                       {
                       ...
                       if (!valid)
                       {
                           error("System level publickey verification failed for {ALGO}",
                                 "ALGO", pqAlgo->name);
                           break;
                        }
                    }
    ...
    const std::string& hashFunc, const std::string& sigSubDir)
{
	...
	fs::path sigFile;
        ...
        else
        {
            ...
            sigFile = fs::path(filePath) / sigSubDir / (bmcImage + SIGNATURE_FILE_EXT);
        }

        // Verify the signature.
        valid = verifyFile(file, sigFile, publicKeyPath, hashFunc);
        if (!valid)
        {
...
bool Signature::verifyPQSignatures(const std::vector<std::string>& imageList)
{
    ...
    return checkAndVerifyImage(imageDirPath, algoPublicKeyFile.string(),
                               imageList, algoFileFound,
                               pqAlgo->hashFunc, pqAlgo->name);
}
