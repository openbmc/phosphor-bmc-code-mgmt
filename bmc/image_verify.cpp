                           ...
                           const std::string& hashFunc)
{
	...
	{
            error("Error ({RC}) occurred during EVP_DigestInit_ex", "RC", ERR_get_error());
            elog<InternalFailure>();
	}

	if (EVP_DigestUpdate(hashCtx.get(), dataPtr(), size) != 1)
        {
            error("Error ({RC}) occurred during EVP_DigestUpdate", "RC", ERR_get_error());
            elog<InternalFailure>();
        }
        ...
        {
            error("Error ({RC}) occurred during EVP_DigestFinal_ex", "RC", ERR_get_error());
            elog<InternalFailure>();
        }
	...
	auto result = EVP_DigestVerifyInit(verifyCtx.get(), nullptr, nullptr,
        ...
        if (result <= 0)
        {
            error("Error ({RC}) occurred during EVP_DigestVerifyInit", "RC", ERR_get_error());
            elog<InternalFailure>();
        }
        ...
        if (result < 0)
        {
            error("Error ({RC}) occurred during EVP_DigestVerify", "RC", ERR_get_error());
            elog<InternalFailure>();
        }

        if (result == 0)
        {
            error("EVP_DigestVerify: ML-DSA signature validation failed on {PATH}", 
                   "PATH", sigFile);
            return false;
        }
