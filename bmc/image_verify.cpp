...
bool Signature::verifyFullImage()
{
    ...
    if (ret)
    {
        ...
        if (fs::exists(mldsaDir, ec2))
        {
            ...
            {
                ret = verifyFileMLDSA(tmpFullFile, mldsaFullImageSig,
                                      mldsaPublicKeyFile);
                if (!ret)
                {
...
bool Signature::verify()
{
    try
    {
        ...
        if (!valid)
        {
            ...
            if (bmcFilesFound && valid)
            {
                if (!verifyMLDSASignatures())
                {
                    error("MLDSA image signature verification failed");
                    return false;
                }
            }
        }
        ...
        for (const auto& optionalImage : optionalImages)
        {
            ...
            if (fs::exists(file, ec))
            {
                ...
                // Verify MLDSA signature for optional image if present
                fs::path mldsaDir(imageDirPath / mldsaDirName);
                if (fs::exists(mldsaDir, ec))
                {
                    ...
                    {
                        bool mldsaValid = verifyFileMLDSA(file, mldsaSigFile,
                                                          mldsaPublicKeyFile);
                        if (!mldsaValid)
                        {
                            error(
                                "MLDSA validation failed for optional image {IMAGE}",
                                "IMAGE", optionalImage);
                            return false;
                        }
                    }
...
inline EVP_PKEY_Ptr Signature::createPublicMLDSA(const fs::path& publicKey)
{
    ...
    if (pkey == nullptr)
    {
        error("Failed to read MLDSA public key from PEM format");
        elog<InternalFailure>();
    }
...
bool Signature::verifyFileMLDSA(const fs::path& file, const fs::path& sigFile,
                                const fs::path& publicKey)
{
    std::error_code ec;
    if (!(fs::exists(file, ec) && fs::exists(sigFile, ec)))
    {
        error("Failed to find MLDSA data or signature file {PATH}", "PATH",
              file);
        if (ec)
        {
            error("Error message: {ERROR_MSG}", "ERROR_MSG", ec.message());
    ...
    if (!mldsaPublicKey)
    {
        error("Failed to create MLDSA public key from {PATH}", "PATH",
              publicKey);
        elog<InternalFailure>();
    }
    ...
    auto result =
        EVP_DigestVerifyInit(mldsaVerifyCtx.get(), nullptr, hashStruct, nullptr,
                             mldsaPublicKey.get());
    if (result <= 0)
    {
...
bool Signature::systemLevelVerifyMLDSA()
{
    ...
    for (const auto& keyType : keyTypes)
    {
        fs::path systemMldsaKeyPath =
            signedConfPath / keyType / mldsaDirName / PUBLICKEY_FILE_NAME;

        if (!fs::exists(systemMldsaKeyPath, ec))
        {
            debug("System MLDSA key not found for {KEYTYPE}, trying next",
                  "KEYTYPE", keyType);
            continue;
        }
        ...
        try
        {
            ...
            if (valid)
            {
                ...
                {
                    debug(
                        "MLDSA system level verification successful with {KEYTYPE}",
                        "KEYTYPE", keyType);
                    break;
                }
            }
...
bool Signature::verifyMLDSASignatures()
{
    ...
    if (!fs::exists(mldsaDir, ec))
    {
        debug("MLDSA directory not found, skipping MLDSA image verification");
        return true;
    }
    ...
    for (const auto& imageName : imageList)
    {
        ...
        if (!fs::exists(mldsaSigFile, ec))
        {
            error("MLDSA signature file not found for {IMAGE}", "IMAGE",
                  imageName);
            return false;
        }

        bool valid =
            verifyFileMLDSA(imageFile, mldsaSigFile, mldsaPublicKeyFile);
        if (!valid)
        {
            error("MLDSA signature validation failed for {IMAGE}", "IMAGE",
                  imageName);
            return false;
        }
    }
