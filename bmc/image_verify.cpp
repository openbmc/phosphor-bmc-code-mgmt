    ...
    try
    {
        ...
        for (const auto& optionalImage : optionalImages)
        {
            ...
            if (fs::exists(file, ec))
            {
                ...
                {
                    std::cout
                        << "Algo Name: " << pqAlgorithm->name << std::endl;
                    std::cout
                        << "hashType: " << pqAlgorithm->hashType << std::endl;
                    fs::path algoDir(imageDirPath / pqAlgorithm->name);
    ...
    const std::string& hashType, const std::string& sigSubDir)
{
    ...
    std::cout << "key: " << publicKeyPath << std::endl;
    std::cout << "folder: " << sigSubDir << std::endl;
    bool valid = true;
...
bool Signature::verifyPQSignatures(const std::vector<std::string>& imageList)
{
    if (!pqAlgorithm.has_value())
    {
        std::cout << "pqAlgorithm empty" << std::endl;
        return false; // No PQ algorithm, skip
    }
    ...
    if (!fs::exists(algoDir, ec))
    {
        std::cout << "MLDSA folder empty" << std::endl;
        return false; // PQ directory doesn't exist, optional
    }
