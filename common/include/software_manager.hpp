...
class SoftwareManager
{
  ...
  protected:
    ...
    virtual std::set<std::string> getConfigurationInterfaces() final;
    std::string getConfigurationInterfaceMuxGpios();

    // e.g. "SPIFlash", "CPLD", "XDPE152C4", "ISL69260", ...
    // This is the 'Type' field in the EM exposes record.
    // Since a code updater can support multiple devices, we return a set
    virtual std::set<std::string> getEMConfigTypes() = 0;
