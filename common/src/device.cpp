...
// NOLINTEND
{
    ...
    void* pldm_pkg =
        pldm_package_util::mmap_pldm_package(image, &pldm_pkg_size);
    ...
    status = pldm_package_util::verifyPLDMPackage(pldm_pkg, pldm_pkg_size);
    ...
    std::shared_ptr<PackageParser> pp =
        pldm_package_util::parsePLDMFWUPPackageComplete(
