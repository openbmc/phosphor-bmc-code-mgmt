    ...
    std::unique_ptr<Software> softwarePendingIn)
{
    ...
    if (!co_await getImageInfo(
            sdbusplus::object_path(softwarePendingIn->objectPath), pldm_pkg,
            pldm_pkg_size, &componentImage, &componentImageSize,
            componentVersion))
    {
        softwarePendingIn->setActivation(ActivationInvalid);
        co_return false;
    ...
    co_await events.generateTargetDetermined(
        sdbusplus::object_path(softwarePending->objectPath), componentVersion);
    ...
    const std::string& componentVersion, RequestedApplyTimes applyTime)
{
    ...
    if (success)
    {
        ...
        co_await events.generateApplyFailed(
            sdbusplus::object_path(softwarePending->objectPath),
