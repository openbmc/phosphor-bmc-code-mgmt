...
GPIOGroup::GPIOGroup(std::vector<std::string> l, std::vector<bool> p)
    : lines_(std::move(l)), polarities_(std::move(p)) {}

GPIOGroup::~GPIOGroup() { releaseAll(); }
...
bool GPIOGroup::muxToBMC() { return mux(false); }
bool GPIOGroup::muxToHost() { return mux(true); }
    ...
    const std::vector<bool>& gpioPolarities, bool inverted)
{
    ...
    for (auto& [chipName, lineNames] : groupLineNames)
    {
        ...
        for (size_t i = 0; i < lineNames.size(); ++i)
        {
            ...
            lg2::info("Requesting chip {CHIP}, GPIO line {LINE} to {VALUE}", "CHIP",
