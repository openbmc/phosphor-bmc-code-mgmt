    ...
    i2cInterface(i2cInterface), originalPage(std::nullopt)
{
    ...
    {
        originalPage = rbuf[0];
        lg2::debug("PageGuard: current Page {PAGE} stored", "PAGE",
                   originalPage.value());
    }
    else
    {
        lg2::error(
            "PageGuard: Failed to read current page, restoration disabled");
    }
}
...
PageGuard::~PageGuard()
{
    if (originalPage.has_value())
    {
        ...
        if (i2cInterface.sendReceive(tbuf, rbuf))
        {
            lg2::debug("PageGuard: restored page {PAGE}", "PAGE", lg2::hex,
                       *originalPage);
        }
        else
        {
