...
Software::Software(sdbusplus::async::context& ctx, const std::string& swid,
                   Device& parent) :
    SoftwareActivation(ctx, Software::createObjectPathWithSwid(swid).c_str()),
    swid(swid), parent(parent), ctx(ctx)
{
    ...
    std::string objPath = Software::createObjectPathWithSwid(swid);

    lg2::debug("{SWID}: created dbus interfaces on path {OBJPATH}", "SWID",
               swid, "OBJPATH", objPath);
};

std::string Software::createObjectPathWithSwid(const std::string& swid)
{
...
sdbusplus::async::task<> Software::createInventoryAssociations(
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    try
    {
        endpoint = co_await parent.config.getInventoryItemObjectPath(ctx);
    }
    ...
    if (isActivating && !endpoint.empty())
    {
    ...
    this->softwareAssociationDefinitions->associations(assocs);
...
void Software::setVersion(const std::string& versionStr)
{
    ...
    this->softwareVersion->version(versionStr);
}
...
void Software::setActivationBlocksTransition(bool enabled)
{
    if (!enabled)
    {
        this->activationBlocksTransition = nullptr;
        return;
    }

    std::string path = this->getObjectPath();
    this->activationBlocksTransition =
...
{
    std::string objPathStr = Software::createObjectPathWithSwid(swid);
    ...
    const std::set<RequestedApplyTimes>& allowedApplyTimes)
{
    ...
    softwareUpdate = std::make_unique<SoftwareUpdate>(
