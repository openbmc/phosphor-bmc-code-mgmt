    ...
    -> sdbusplus::async::task<start_update_t::return_type>
{
    ...
    softwareUpdateExternal->setActivation(
        ActivationInterface::Activations::NotReady);

    std::string newObjPath = softwareUpdateExternal->getObjectPath();
        ...
        [](Device& device, int imageDup, RequestedApplyTimes applyTime,
           std::unique_ptr<Software> swupdate) -> sdbusplus::async::task<> {
            co_await device.startUpdateAsync(imageDup, applyTime,
                                             std::move(swupdate));
            co_return;
        }(device, imageDup, applyTime, std::move(softwareUpdateExternal)));
    // NOLINTEND
