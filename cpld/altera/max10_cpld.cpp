...
#include <xyz/openbmc_project/Configuration/Max10CPLD/client.hpp>
#include <xyz/openbmc_project/ObjectMapper/client.hpp>

using Max10ConfigClient = sdbusplus::client::xyz::openbmc_project::configuration::Max10CPLD<>;
using ObjectMapperClient = sdbusplus::client::xyz::openbmc_project::ObjectMapper<>;
...
sdbusplus::async::task<std::string> Max10CPLD::getUpdateGpioNameFromDBus()
{
    try
    {
        auto mapper = ObjectMapperClient(ctx_)
                          .service(ObjectMapperClient::default_service)
                          .path(ObjectMapperClient::instance_path);

        auto subtree = co_await mapper.get_sub_tree(
            "/xyz/openbmc_project/inventory", 0,
            std::vector<std::string>{Max10ConfigClient::interface});

        for (const auto& [path, services] : subtree)
        {
            for (const auto& [service, interfaces] : services)
            {
                auto config = Max10ConfigClient(ctx_)
                                  .service(service)
                                  .path(path);

                uint64_t configBus = co_await config.bus();
                uint64_t configAddr = co_await config.address();

                if (configBus == static_cast<uint64_t>(this->bus_) &&
                    configAddr == static_cast<uint64_t>(this->address_))
                {
                    co_return co_await config.update_gpio();
                }
            }
        }
    }
    ...
    std::function<bool(int)> progressCallBack)
{
    ...
    std::string gpioName = co_await getUpdateGpioNameFromDBus();
