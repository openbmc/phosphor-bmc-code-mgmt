...
//#define DEBUG
    ...
    #define LG2_DEBUG(fmt, ...) \
        lg2::info("[{SOURCE_FUNC}] " fmt, \
                   "SOURCE_FUNC", std::string(__func__), \
                   ##__VA_ARGS__)
#else
     #define LG2_DEBUG(fmt, ...) do {} while (0)
#endif
...
Max10CPLD::Max10CPLD(sdbusplus::async::context& ctx, const uint16_t bus,
    ...
    ctx_(ctx),
    bus_(bus), address_(address), chip_(chip), target_(target),
...
bool Max10CPLD::ensureOpen()
{
    ...
    {
        lg2::error("Failed to set I2C address 0x{ADDR:02X}: {ERR}", "ADDR", address_,
...
std::string Max10CPLD::getUpdateGpioNameFromDBus() const
{
    try
    {
        ...
        mapperCall.append("/xyz/openbmc_project/inventory", 0,
                          std::vector<std::string>{"xyz.openbmc_project.Configuration.Max10Cpld"});
        ...
        for (const auto& [path, services] : subtree)
        {
            ...
            {
                auto getCall = bus.new_method_call(service.c_str(), path.c_str(),
                                                   "org.freedesktop.DBus.Properties", "GetAll");
                getCall.append("xyz.openbmc_project.Configuration.Max10Cpld");

                auto getReply = bus.call(getCall);
                std::map<std::string, std::variant<uint64_t, std::string>> props;
                getReply.read(props);
                ...
                if (props.contains("Bus") && props.contains("Address"))
                {
                    ...
                    } else if (std::holds_alternative<std::string>(props.at("Address"))) {
                        cfgAddr = std::stoull(std::get<std::string>(props.at("Address")), nullptr, 0);
                    }
                    ...
                    if (cfgBus == bus_ && cfgAddr == address_)
                    {
                        ...
                        {
                            return std::get<std::string>(props.at("UpdateGpio"));
                        }
                    }
                }
            }
    ...
    {
        lg2::error("Failed to query UpdateGpio from D-Bus: {ERR}", "ERR", e.what());
    }
    return "";
}
...
bool Max10CPLD::readReg(uint32_t reg, uint32_t& value)
{
    ...
    lg2::error("I2C read reg {REG} failed: {ERR}", "REG", regStr,
               "ERR", std::strerror(errno));
    return false;
}
...
bool Max10CPLD::writeReg(uint32_t reg, uint32_t value)
{
    ...
    char regStr[32];
    snprintf(regStr, sizeof(regStr), "0x%X", reg);
    lg2::error("I2C write reg {REG} failed via ioctl: {ERR}", 
               "REG", regStr, "ERR", std::strerror(errno));
    return false;
}
...
bool Max10CPLD::unprotectSector(int sectorId)
{
    ...
    switch (sectorId)
    {
        case 5: ctrl &= ~protectSec5; break;
        case 4: ctrl &= ~protectSec4; break;
        case 3: ctrl &= ~protectSec3; break;
        case 2: ctrl &= ~protectSec2; break;
        case 1: ctrl &= ~protectSec1; break;
        case 0: ctrl = 0xFFFFFFFF; break;
        default: 
            lg2::error("Unknown sector id {ID}", "ID", sectorId);
            return false;
    }
    LG2_DEBUG("unprotect {SECTORID} Write {CTRL}", "SECTORID", sectorId, "CTRL", toHex8(ctrl));
...
bool Max10CPLD::eraseSector(int sectorId)
{
    ...
    switch (sectorId)
    {
        case 5: ctrl |= sectorErase5; break;
        case 4: ctrl |= sectorErase4; break;
        case 3: ctrl |= sectorErase3; break;
        case 2: ctrl |= sectorErase2; break;
        case 1: ctrl |= sectorErase1; break;
        case 0: ctrl |= sectorEraseNone; break;
        default:
            lg2::error("Unknown sector id {ID}", "ID", sectorId);
            return false;
    }

    LG2_DEBUG("erase {SECTORID} Write {CTRL}", "SECTORID", sectorId, "CTRL", toHex8(ctrl));
    if (!writeReg(profile_.csrBase + 0x04, ctrl))
    {
...
bool Max10CPLD::waitEraseDone(int timeoutCount)
{
    ...
    for (int cnt = 0; cnt < timeoutCount; ++cnt)
    {
        ...
        if (status != 0)
        {
            lg2::error("Erase failed, status={STATUS}", "STATUS", toHex8(status));
            return false;
        }
    }
...
bool Max10CPLD::waitWriteDone(int timeoutCount)
{
    for (int cnt = 0; cnt < timeoutCount; ++cnt)
    {
        ...
        if (status != 0)
        {
            lg2::error("Write failed, status={STATUS}", "STATUS", toHex8(status));
            return false;
        }
    }
                           ...
                           std::function<bool(int)> progressCallBack)
{
    ...
    LG2_DEBUG("MAX10 Update: imageSize={SIZE}, type={TYPE}", 
              "SIZE", imageSize, "TYPE", profile_.imageType);

    const auto expectedSize = static_cast<size_t>(profile_.endAddr - profile_.startAddr);
    if (imageSize != expectedSize)
    {
        lg2::error("Image size mismatch: got {GOT}, expect {EXP}", 
                   "GOT", imageSize, "EXP", expectedSize);
        return false;
    }
         ...
         addr += 4, offset += 4)
    {
        ...
        LG2_DEBUG("write add:{ADDRESS} word:{WORD}", "ADDRESS", toHex8(profile_.dataBase + addr), "WORD", toHex8(word));
    ...
    std::function<bool(int)> progressCallBack)
{
    ...
    if (!gpioName.empty())
    {
        ...
        if (updateGpio)
        {
            try
            {
                ...
                updateGpio.request({"max10-update", gpiod::line_request::DIRECTION_OUTPUT, 0}, 1);
                LG2_DEBUG("GPIO {GPIO} has been set high to enable update mode", "GPIO", gpioName);
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
				
				int readback = updateGpio.get_value();
                LG2_DEBUG("After requesting GPIO {GPIO}, the readback value is {VAL}", "GPIO", gpioName, "VAL", readback);

                if (readback != 1)
                {
                    lg2::error("GPIO {GPIO} readback does not match the expected value: expect={EXP}, got={VAL}",
    ...
    if (updateGpio)
    {
        ...
        updateGpio.release();
        LG2_DEBUG("GPIO {GPIO} has been set low to exit update mode", "GPIO", gpioName);
    }
