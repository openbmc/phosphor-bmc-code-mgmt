...
#include <xyz/openbmc_project/Configuration/CPLDFirmware/client.hpp>
...
using AlteraConfigClient = sdbusplus::async::client_t<
    sdbusplus::client::xyz::openbmc_project::configuration::CPLDFirmware>;
...
Max10CPLD::Max10CPLD(sdbusplus::async::context& ctx, const uint16_t bus,
    ...
    ctx_(ctx), bus_(bus), address_(address), chip_(chip),
    configType_(configType), debugMode_(debugMode), profile_(profile)
{}
...
sdbusplus::async::task<std::string> Max10CPLD::getUpdateGpioNameFromDBus()
{
    try
    {
        constexpr auto interface = AlteraConfigClient::interface;
        ...
        auto subtree = co_await mapper.call<SubTreeType>(
            ctx_, "GetSubTree", "/xyz/openbmc_project/inventory", 0,
        ...
        const auto& [path, services] = *subtree.begin();
        if (services.empty())
        {
            co_return "";
        }
        ...
        auto client = AlteraConfigClient(ctx_).service(service).path(path);

        co_return co_await client.update_gpio();
    }
    ...
    std::function<bool(int)> progressCallBack)
{
    ...
    auto gpioName = co_await getUpdateGpioNameFromDBus();
