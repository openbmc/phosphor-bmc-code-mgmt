...
using Max10ConfigClient =
    sdbusplus::client::xyz::openbmc_project::configuration::Max10CPLD<>;
using ObjectMapperClient =
...
sdbusplus::async::task<std::string> Max10CPLD::getUpdateGpioNameFromDBus()
{
    try
    {
        ...
        for (const auto& [path, services] : subtree)
        {
            ...
            {
                auto config =
                    Max10ConfigClient(ctx_).service(service).path(path);
