...
{
    const auto chipModelName = getMax10ChipName(chipEnum, max10Type::model);
    const auto configTypeName = getMax10ChipName(chipEnum, max10Type::type);
    ...
    return std::make_unique<Max10CPLD>(ctx, bus, address, chipModelName,
                                       configTypeName, false,
                                       loadProfileFromEnv());
}
...
const bool vendorRegistered = [] {
    ...
    {
        (void)info;
        const auto typeName = getMax10ChipName(chipEnum, max10Type::type);
