...
{
    const auto chipModelName = getMax10ModuleName(chipEnum);
    const auto configTypeName = getMax10ConfigType(chipEnum);
...
const bool vendorRegistered = [] {
    ...
    {
        (void)info;
        const auto typeName = getMax10ConfigType(chipEnum);
        if (typeName.empty())
        {
            continue;
        }
