...
Max10Profile loadProfileFromEnv()
{
    ...
    if (auto v = parseEnvUnsigned<uint32_t>("MAX10_CSR_BASE")) p.csrBase = *v;
    if (auto v = parseEnvUnsigned<uint32_t>("MAX10_DATA_BASE")) p.dataBase = *v;
    if (auto v = parseEnvUnsigned<uint32_t>("MAX10_BOOT_BASE")) p.bootBase = *v;
    if (auto v = parseEnvUnsigned<uint32_t>("MAX10_CFM_START")) p.startAddr = *v;
    if (auto v = parseEnvUnsigned<uint32_t>("MAX10_CFM_END")) p.endAddr = *v;
    if (auto v = parseEnvUnsigned<uint8_t>("MAX10_IMAGE_TYPE")) p.imageType = *v;
...
{
    const auto chipModelStr = getMax10ChipStr(chipEnum, max10StringType::modelString);
...
sdbusplus::async::task<bool>
    Max10CPLDFactory::updateFirmware(bool /*force*/, const uint8_t* image,
                                     size_t imageSize,
    ...
    for (const auto& [chipEnum, info] : getSupportedDeviceMap())
    {
        ...
        CPLDFactory::instance().registerCPLD(
            ...
            [chipEnum](sdbusplus::async::context& ctx,
                       ...
                       uint8_t address) {
                return std::make_unique<Max10CPLDFactory>(ctx, chipName,
                                                          chipEnum, bus,
                                                          address);
            });
    }
