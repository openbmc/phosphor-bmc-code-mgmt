...
// Progress bar and buffer constants
constexpr int progressBasePct = 10;
constexpr int progressProgramPct = 85;
constexpr int progressCompletePct = 100;
constexpr size_t progressUpdateInterval = 1024;
    ...
    const std::function<bool(int)>& progressCallback)
{
    ...
    if (progressCallback)
    {
        progressCallback(progressBasePct);
    }
         ...
         addr += wordSize, offset += wordSize)
    {
        ...
        if (progressCallback && (offset % progressUpdateInterval == 0))
        {
            const int pct =
                progressBasePct +
                static_cast<int>((offset * progressProgramPct) / imageSize);
    ...
    if (progressCallback)
    {
        progressCallback(progressCompletePct);
    }
