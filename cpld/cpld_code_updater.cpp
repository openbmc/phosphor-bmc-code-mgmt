...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    std::optional<std::string> chipVendor =
        co_await dbusGetRequiredProperty<std::string>(ctx, service, path, configIface, "ChipVendor");
    std::optional<std::string> chipName =
        co_await dbusGetRequiredProperty<std::string>(ctx, service, path, configIface, "ChipName");

    if (!busNo.has_value() || !address.has_value() ||
        !chipVendor.has_value() || !chipName.has_value())
    {
    ...
    // Check correctness
    lg2::debug(
        "[config] Voltage regulator device type: {TYPE} - {NAME} on Bus: {BUS} at Address: {ADDR}",
        "TYPE", chipVendor.value(), "NAME", chipName.value(),
        "BUS", busNo.value(), "ADDR", address.value());

    auto cpld = std::make_unique<phosphor::software::cpld::device::CPLD>(
        ctx, chipVendor.value(), chipName.value(), busNo.value(), address.value(),
        config, this);

    std::string version = "unknown";

    std::unique_ptr<Software> software =
        std::make_unique<Software>(ctx, *cpld);
