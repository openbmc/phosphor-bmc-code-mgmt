    ...
    const std::string& service, const std::string& path, SoftwareConfig& config)
{
            ...
            for (size_t i = 0; true; i++)
            {
                ...
                std::optional<std::string> name =
                    co_await dbusGetRequiredProperty<std::string>(
                        ctx, service, path, iface, "Name");
                std::optional<std::string> polarity =
                    co_await dbusGetRequiredProperty<std::string>(
                        ctx, service, path, iface, "Polarity");
                ...
                info("MuxOutput{NUM} name: {NAME}, value: {VAL}", "NUM",
                     std::to_string(i), "NAME", gpioLines[i], "VAL",
                     gpioValues[i]);
            }
        }
    }
    ...
    auto cpld = std::make_unique<CPLDDevice>(
        ...
        protocolStr, jtagIndexStr, gpioLines, gpioValues, config, this);
