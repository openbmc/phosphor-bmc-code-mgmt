...
bool CPLD::latticeUpdate(const uint8_t* image, size_t image_size)
{
    // Update the lattice device
    auto cpldManager = CpldLatticeManager(bus, address, image, image_size,
...
const std::map<std::string, std::vector<uint8_t>> xo2xo3DeviceId_t = {
    ...
    {"LCMXO3LF-4300C", {0xe1, 0x2b, 0xc0, 0x43}},
    {"LCMXO3LF-6900", {0x61, 0x2b, 0xd0, 0x43}},
    {"LCMXO3D-4300", {0x01, 0x2e, 0x20, 0x43}},
    {"LCMXO3D-9400", {0x21, 0x2e, 0x30, 0x43}},
};

static int findNumberSize(const std::string& start, const std::string& end,
...
int CpldLatticeManager::jedFileParser()
{
    ...
    while (getline(iss, line))
    {
        if (line.rfind(TAG_QF, 0) == 0)
        {
            ...
            static constexpr auto start = sizeof(TAG_QF) - 1;
        ...
        else if ((checksumStart) && (line.size() != 1))
        {
            ...
            static constexpr auto start = sizeof(TAG_CHECKSUM) - 1;
        ...
        else if (versionStart)
        {
            if ((line.rfind(TAG_USERCODE, 0)) && (line.size() != 1))
            {
                ...
                if (line.rfind(TAG_UH, 0) == 0)
                {
                    ...
                    static constexpr auto start = sizeof(TAG_UH) - 1;
                    std::istringstream iss(line.substr(start, numberSize));
    ...
    */
    ...
    for (size_t i = 0; i < fwInfo.cfgData.size(); i += iterSize)
    {
        ...
        data.insert(data.end(),
            fwInfo.cfgData.begin() + static_cast<std::ptrdiff_t>(i),
    ...
    for (int b_idx = 0; b_idx < XO5_CFG_BLOCK_NUM; ++b_idx)
    {
        ...
        for (int p_idx = 0; p_idx < XO5_PAGE_NUM; ++p_idx)
        {
            if (data_offset < fwInfo.cfgData.size())
            {
                ...
                float progressRate =
                    (static_cast<float>(data_offset) / fwInfo.cfgData.size()) *
                    100;
                std::cout << "Update :" << std::fixed << std::dec
                          << std::setprecision(2) << progressRate << "% \r";
...
bool XO5I2CManager::verifyCfgData(uint8_t cfg)
{
    ...
    for (int b_idx = 0; b_idx < XO5_CFG_BLOCK_NUM; ++b_idx)
    {
        ...
        for (int p_idx = 0; p_idx < XO5_PAGE_NUM; ++p_idx)
        {
            ...
            float progressRate =
                (static_cast<float>(data_offset) / fwInfo.cfgData.size()) * 100;
...
int CpldLatticeManager::XO5Family_update()
{
    ...
    std::cout << "Program CFG"
              << (operate_taget == i2cManager.XO5_PARTITION_CFG0 ? "0" : "1")
              << " data ..." << std::endl;
