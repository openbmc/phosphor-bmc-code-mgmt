...
sdbusplus::async::task<bool> CpldLatticeManager::resetConfigFlash()
{
    ...
    co_return co_await i2cInterface.sendReceive(command.data(), command.size(),
                                                nullptr, 0);
}
...
sdbusplus::async::task<bool> CpldLatticeManager::disableConfigInterface()
{
    ...
    bool success = co_await stdexec::starts_on(
        sched, i2cInterface.sendReceive(command.data(), command.size(), nullptr, 0));
    ...
    std::function<bool(int)> progressCallBack)
{
    ...
    if (!(co_await stdexec::starts_on(sched, disableConfigInterface())))
    {
