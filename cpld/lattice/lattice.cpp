...
int CpldLatticeManager::jedFileParser()
{
    ...
    while (getline(iss, line))
    {
        ...
        if (cfStart)
        {
            ...
            if ((line.rfind(TAG_CF_START, 0)) && (line.size() != 1))
            {
                ...
                {
                    lg2::info("CF size = {CF}", "CF", fwInfo.cfgData.size());
                    cfStart = false;
                    if (!ebrInitDataStart)
                    {
                        ufmPrepare = true;
        ...
        else if (ebrInitDataStart)
        {
            ...
            if ((line.rfind(TAG_EBR_INIT_DATA, 0)) && (line.size() != 1))
            {
                if ((line.rfind('L', 0)) && (line.size() != 1))
                {
                    ...
                    {
                        lg2::info("CF size with EBR_INIT Data = {CF}", "CF",
                                  fwInfo.cfgData.size());
        ...
        else if (ufmStart)
        {
            if ((line.rfind('L', 0)) && (line.size() != 1))
            {
                ...
                {
                    lg2::info("UFM size = {UFM}", "UFM", fwInfo.ufmData.size());
                    ufmStart = false;
                }
            }
        }
    ...
    lg2::info("jedFileCheckSum = {JEDFILECHECKSUM}", "JEDFILECHECKSUM",
              jedFileCheckSum);
    ...
    lg2::info("JED File CheckSum = {JEDFILECHECKSUM}", "JEDFILECHECKSUM",
              jedFileCheckSum);
...
// NOLINTEND(readability-static-accessed-through-instance)
{
        ...
        chipWantToUpdate->second == readData)
    {
        ...
        lg2::info("Device ID match with chip. Chip name: {CHIPNAME}",
                  "CHIPNAME", chip);
        co_return true;
    }
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    auto cfgnum =
        operate_taget == XO5I2CManager::XO5_PARTITION_CFG0 ? "0" : "1";
    lg2::info("Program CFG{CFGNUM} data ...", "CFGNUM", cfgnum);
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    if (target.empty())
    {
        ...
        lg2::info("CPLD version: {VERSION}", "VERSION", userCode);
    }
    else if (target == "CFG0" || target == "CFG1")
    {
        ...
        lg2::info("CPLD {TARGET} version: {VERSION}", "TARGET", target,
                  "VERSION", userCode);
    }
    else
    {
