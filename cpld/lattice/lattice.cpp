...
int CpldLatticeManager::jedFileParser()
{
    ...
    while (getline(iss, line))
    {
        if (line.rfind(TAG_QF, 0) == 0)
        {
            ...
            lg2::info("QF Size = {QF}", "QF", fwInfo.QF);
        }
        else if (line.rfind(TAG_CF_START, 0) == 0)
        {
            cfStart = true;
        }
        else if (ENABLE_UPDATE_EBR_INIT && line.rfind(TAG_EBR_INIT_DATA, 0) == 0)
        {
        ...
        if (line.rfind("NOTE DEVICE NAME:", 0) == 0)
        {
            ...
            {
                lg2::info("[OK] The image device name match with chip name");
            }
            else
            {
                lg2::info("STOP UPDATEING: The image not match with chip.");
                return -1;
            }
        }
        ...
        if (cfStart)
        {
            ...
            if ((line.rfind(TAG_CF_START, 0)) && (line.size() != 1))
            {
                ...
                {
                    lg2::info(
                        "CF size = {CF}", "CF", fwInfo.cfgData.size());
                    cfStart = false;
                    if (!ebrInitDataStart)
                    {
                        ufmPrepare = true;
        ...
        else if (ebrInitDataStart)
        {
            ...
            if ((line.rfind(TAG_EBR_INIT_DATA, 0)) && (line.size() != 1))
            {
                if ((line.rfind('L', 0)) && (line.size() != 1))
                {
                    ...
                    {
                        lg2::info(
                            "CF size with EBR_INIT Data = {CF}", "CF",
                            fwInfo.cfgData.size());
        ...
        else if ((checksumStart) && (line.size() != 1))
        {
            ...
            lg2::info("Checksum = {CHECKSUM}", "CHECKSUM", fwInfo.CheckSum);
        }
        else if (versionStart)
        {
            if ((line.rfind(TAG_USERCODE, 0)) && (line.size() != 1))
            {
                ...
                if (line.rfind(TAG_UH, 0) == 0)
                {
                    ...
                    lg2::info("UserCode = {USERCODE}", "USERCODE",
                              fwInfo.Version);
                }
            }
        }
        else if (ufmStart)
        {
            if ((line.rfind('L', 0)) && (line.size() != 1))
            {
                ...
                {
                    lg2::info(
                        "UFM size = {UFM}", "UFM", fwInfo.ufmData.size());
                    ufmStart = false;
                }
            }
        }
    ...
    lg2::info("jedFileCheckSum = {JEDFILECHECKSUM}",
    ...
    lg2::info(
        "JED File CheckSum = {JEDFILECHECKSUM}", "JEDFILECHECKSUM",
        jedFileCheckSum);
...
// NOLINTEND(readability-static-accessed-through-instance)
{
        ...
        chipWantToUpdate->second == readData)
    {
        ...
        lg2::info(
            "Device ID match with chip. Chip name: {CHIPNAME}",
            "CHIPNAME", chip);
        co_return true;
    }
...
// NOLINTEND(readability-static-accessed-through-instance)
{
        ...
        ((statusReg >> failOrOKBit) & 1) == isOK)
    {
        if (debugMode)
        {
            lg2::info("Status Reg : OK");
        }
    }
    else
    {
        lg2::error(
            "Status Reg : Fail! Please check the I2C bus and address.");
        co_return false;
    }
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    lg2::info("Starts to update ...");
    lg2::info("Enable program mode.");
    ...
    lg2::info("Write program page ...");
    if (!(co_await writeProgramPage()))
    {
        lg2::error("Write program page failed.");
        co_return false;
    }
    lg2::info("Write program page done.");

    lg2::info("Program user code.");
    ...
    lg2::info("Disable config interface.");
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    lg2::info("Verification successful.");
    co_return true;
}
...
int XO5I2CManager::XO5jedFileParser()
{
    ...
    while (getline(iss, line))
    {
        if (line.rfind(TAG_QF, 0) == 0)
        {
            ...
            lg2::info("QF Size = {QFSIZE}", "QFSIZE", fwInfo.QF);
        }
        else if (line.rfind(TAG_CF_START, 0) == 0)
        {
            cfStart = true;
    ...
    {
        lg2::info("[OK] JED File Checksum compare success" );
    }
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    lg2::info("Starting to update {CHIPNAME}", "CHIPNAME", chip);
    ...
    auto cfgnum = operate_taget == XO5I2CManager::XO5_PARTITION_CFG0 ? "0" : "1";
    lg2::info(
        "Program CFG{CFGNUM} data ...",
        "CFGNUM", cfgnum);
    ...
    lg2::info("Update completed! Please AC.");
    co_return true;
}
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    if (target.empty())
    {
        ...
        lg2::info(
            "CPLD version: {VERSION}", "VERSION", userCode);
    }
    else if (target == "CFG0" || target == "CFG1")
    {
        ...
        lg2::info(
            "CPLD {TARGET} version: {VERSION}", "TARGET", target,
            "VERSION", userCode);
    }
    else
    {
