...
static constexpr std::string_view TAG_UFM = "NOTE TAG DATA";
static constexpr std::string_view TAG_ROW = "NOTE FEATURE";
...
class CpldLatticeManager : public CpldManager
{
  public:
    ...
    cpldI2cInfo fwInfo{};
    CpldLatticeManager(const uint8_t bus, const uint8_t address,
        ...
        CpldManager(bus, address, image, imageSize, chip, interface, target,
                    debugMode),
...
class XO5I2CManager : public CpldLatticeManager
{
  ...
  public:
    ...
    XO5I2CManager(const uint8_t bus, const uint8_t addr, const uint8_t* image,
                  size_t imageSize, const std::string& chip,
                  const std::string& interface, const std::string& target,
                  const bool debugMode) :
        CpldLatticeManager(bus, addr, image, imageSize, chip, interface, target,
                           debugMode)
    {}
