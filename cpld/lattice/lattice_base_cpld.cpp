...
std::string LatticeBaseCPLDJtag::findActionInFile(const std::string path)
{
    ...
    while (pos < content.size() && std::isspace(
        static_cast<unsigned char>(content[pos])))
    {
        ++pos;
    }
        ...
        std::function<bool(int)> progressCallBack)
{
    ...
    const std::string path = "/tmp/cpld-image-" +
                             std::to_string(Software::getRandomId())
                             + ".bin";
