...
{
  public:
    LatticeBaseCPLDJtag(sdbusplus::async::context& ctx,
                    ...
                    ctx(ctx), jtagIndex(jtagIndex), 
                    chipName(chipName), chipModel(chipModel),
    ...
    sdbusplus::async::task<bool> updateFirmware(
        const uint8_t* image, size_t imageSize,
        std::function<bool(int)> progressCallBack);
    virtual sdbusplus::async::task<bool> getVersion(
        std::string& version) = 0;
    std::string findActionInFile(const std::string path);
