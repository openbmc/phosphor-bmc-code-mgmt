...
constexpr std::chrono::microseconds readyPollInterval(100);
...
static std::optional<std::vector<uint8_t>> calculate_sha2_384_openssl(
...
static uint8_t getCfgIdx(std::string_view target)
{
    ...
    return 1; // default to CFG0
}

sdbusplus::async::task<bool> LatticeXO5_65TCPLD::waitUntilReady(
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::toggle_crc16(bool enable)
{
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::read_crc_status(bool& crc_ok)
{
    std::vector<uint8_t> request = {
        static_cast<uint8_t>(xo5Cmd::checkBusyStatus), 0x00, 0x00, 0x00};
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::sendReceive(
    const std::vector<uint8_t>& request, std::vector<uint8_t>& response)
{
    ...
    {
        // controlCmdCrc, checkBusyStatus, readStatusReg, refresh
        // commands are always sent without CRC16
        crc16Enabled = false;
    }
    else
    {
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::eraseCfg(uint8_t cfgIdx)
{
    std::vector<uint8_t> emptyResponse = {};

    std::vector<std::vector<uint8_t>> cmds = {
        {static_cast<uint8_t>(xo5Cmd::enablex), 0x0, 0x0, 0x0},
        {static_cast<uint8_t>(xo5Cmd::initAddress), 0x0, cfgIdx, 0x0},
        {static_cast<uint8_t>(xo5Cmd::erase), 0x0, cfgIdx, 0x0}};
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::programCfg(
    uint8_t cfgIdx, const std::vector<uint8_t>& data_in)
{
    std::vector<uint8_t> emptyResponse = {};
    std::vector<uint8_t> response;
    // pre-write commands
    ...
    std::vector<uint8_t> cfgData = data_in;
    ...
    for (const auto& cmd : postWriteCmds)
    {
        if (!(co_await sendReceive(cmd, emptyResponse)))
        {
            lg2::error("Failed to send command opcode: {OPCODE:X}", "OPCODE",
                       lg2::hex, cmd[0]);
            co_return false;
        }
    }
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::verifyCfg(
    const std::vector<uint8_t>& cfgData)
{
    ...
    if (!(co_await sendReceive(
            {static_cast<uint8_t>(xo5Cmd::readhash), 0x0, 0x0, 0x0}, rxDigest)))
    {
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::readUserCode(uint32_t& userCode)
{
    ...
    userCode = (response[3] << 24) | (response[2] << 16) | (response[1] << 8) |
               (response[0]);
    // lg2::debug("User Code: {USER_CODE}", "USER_CODE", lg2::hex, userCode);
    co_return true;
}

sdbusplus::async::task<bool> LatticeXO5_65TCPLD::readSoftIPId()
{
    // read device ID
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::prepareUpdate(
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::doErase()
{
    lg2::debug("Erasing {TARGET}...", "TARGET", target);
    if (target == "CFG0")
    {
        // backup header before erasing CFG0
    ...
    if (!(co_await eraseCfg(getCfgIdx(target))))
    {
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::doUpdate()
{
    lg2::debug("Programming {TARGET}...", "TARGET", target);
    if (!(co_await programCfg(getCfgIdx(target), fwInfo.cfgData)))
    {
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::finishUpdate()
{
    if (!(co_await verifyCfg(fwInfo.cfgData)))
    {
        lg2::error("Verify cfg data failed.");
        co_return false;
    }
    // restore header after erasing CFG0
    if (target == "CFG0")
    {
...
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::readCfg(
    uint8_t targetCfg, std::vector<uint8_t>& data_out, uint32_t bytes_to_read)
{
    // Pad bytes_to_read to a multiple of incrSize if necessary
    uint32_t padded_bytes_to_read = bytes_to_read;
    if (padded_bytes_to_read % xo5Cfg::incrDataSize != 0)
    {
        padded_bytes_to_read += xo5Cfg::incrDataSize -
                                (padded_bytes_to_read % xo5Cfg::incrDataSize);
    }
    data_out.clear();
    data_out.reserve(padded_bytes_to_read);
    ...
    // loop reading xo5Cfg::incrDataSize bytes using repeated LSC_READ_INCR
    for (uint32_t offset = 0; offset < padded_bytes_to_read;
         offset += xo5Cfg::incrDataSize)
    {
        ...
        data_out.insert(data_out.end(), response.begin(), response.end());
    }
...
// backup header CFG to reserved UFM when erasing CFG0
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::backupHeader()
{
    // read header data (4K)
    std::vector<uint8_t> header_data;
    if (!(co_await readCfg(xo5Cfg::headerIdx, header_data, 0x1000)))
    {
        lg2::error("Failed to read HEADER data");
        co_return false;
    }
    // skip backup if header_data is all 0xFF
    if (std::all_of(header_data.begin(), header_data.end(),
                    [](uint8_t b) { return b == 0xFF; }))
    {
        lg2::debug("HEADER data is empty, skipping backup.");
        co_return true;
    }
    // erase reserved UFM before writing header backup
    ...
    // write header data to reserved UFM
    lg2::debug("Writing HEADER backup to reserved UFM ...");
    if (!(co_await programCfg(xo5Cfg::ufm8Idx, header_data)))
    {
...
// restore header from reserved UFM after program CFG0
sdbusplus::async::task<bool> LatticeXO5_65TCPLD::restoreHeader()
{
    // read header backup 4K data from reserved UFM
    std::vector<uint8_t> header_data;
    if (!(co_await readCfg(xo5Cfg::ufm8Idx, header_data, 0x1000)))
    {
    ...
    // write header data to HEADER area
    lg2::debug("Writing HEADER data to HEADER area ...");
    if (!(co_await programCfg(xo5Cfg::headerIdx, header_data)))
    {
