    ...
    std::string& version)
{
    ...
    auto success = co_await i2cInterface.sendReceive(&versionReg, regLength,
                                                     readBuf, verLength);
    if (!success)
    {
    ...
    oss << std::hex << std::setfill('0') << std::setw(2)
        << static_cast<int>(readBuf[0]) << "." << std::setw(2)
        << static_cast<int>(readBuf[1]) << "." << std::setw(2)
        << static_cast<int>(readBuf[2]);
