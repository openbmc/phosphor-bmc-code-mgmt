...
bool LatticeXO5CPLD::isCrcRequired(uint8_t opcode) const
{
    ...
    bool isV20 = (softIpVersion & 0xF0) == 0x20;
...
sdbusplus::async::task<bool> LatticeXO5CPLD::isCrcSuccessful(
...
sdbusplus::async::task<bool> LatticeXO5CPLD::programStandard(
    uint8_t cfgIndex, const std::vector<uint8_t>& cfgData)
{
...
sdbusplus::async::task<bool> LatticeXO5CPLD::programTSeries(
    uint8_t cfgIndex, const std::vector<uint8_t>& cfgData)
{
    if (!(co_await lockI2C()))
    {
    ...
    {
        paddedData.insert(paddedData.end(), chunkSize - (totalBytes % chunkSize),
                       0xFF);
    ...
    std::optional<uint8_t> setIdx, const std::vector<uint8_t>* customData)
{
    ...
    if (isTSeries)
    {
        co_return co_await programTSeries(cfgIndex, cfgData);
    }

    co_return co_await programStandard(cfgIndex, cfgData);
}
...
sdbusplus::async::task<bool> LatticeXO5CPLD::verifyStandardCfg()
{
    using diff_t = std::vector<uint8_t>::difference_type;
    const auto& cfgData = fwInfo.cfgData;
    const size_t totalBytes = cfgData.size();
    ...
    const size_t endBlock = startBlock + xo5Cfg::blocksPerCfg;
...
sdbusplus::async::task<bool> LatticeXO5CPLD::verifyTSeriesCfg()
{
    if (!(co_await lockI2C()))
    {
        co_return false;
...
sdbusplus::async::task<bool> LatticeXO5CPLD::verifyCfg()
{
    if (isTSeries)
    {
        co_return co_await verifyTSeriesCfg();
    }
    co_return co_await verifyStandardCfg();
}
