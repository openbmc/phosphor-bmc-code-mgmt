...
class LatticeXO5CPLD : public LatticeBaseCPLD
{
  ...
  private:
    ...
    sdbusplus::async::task<bool> programStandard(
        uint8_t cfgIndex, const std::vector<uint8_t>& cfgData);
    sdbusplus::async::task<bool> programTSeries(
        uint8_t cfgIndex, const std::vector<uint8_t>& cfgData);
    ...
    sdbusplus::async::task<bool> verifyStandardCfg();
    sdbusplus::async::task<bool> verifyTSeriesCfg();
