...
class LatticeXO5CPLD2 : public LatticeBaseCPLD
{
  ...
  private:
    ...
    sdbusplus::async::task<bool> programCfg(uint8_t cfgIdx, const std::vector<uint8_t>& cfgData);
    sdbusplus::async::task<bool> verifyCfg(const std::vector<uint8_t>& cfgData);
    sdbusplus::async::task<bool> readCfg(uint8_t cfgIdx, std::vector<uint8_t>& data_out, uint32_t bytes_to_read);
