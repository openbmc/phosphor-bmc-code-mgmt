...
sdbusplus::async::task<bool> LatticeXO5TSeriesCPLD::waitUntilReady(
    std::chrono::milliseconds timeout)
{
    ...
    while (std::chrono::steady_clock::now() < endTime)
    {
        ...
        co_await sdbusplus::async::sleep_for(ctx, tSeriesReadyPollInterval);
    }
    co_return false;
}
