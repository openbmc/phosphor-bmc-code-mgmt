...
uint8_t LatticeXO5DCPLD::checkSum(std::vector<uint8_t>& vec)
{
    uint8_t checkSum = 0;

    for (std::vector<uint8_t>::iterator it = vec.begin(); it != (vec.end() - 1);
         it++)
    {
        checkSum += (*it);
    }
    ...
    uint8_t cmdId, std::vector<uint8_t>& data)
{
    ...
    std::copy(response.begin() + RECEIVE_DATA_POS,
              response.begin() + RECEIVE_DATA_POS + data.size(), data.begin());
    co_return true;
}
...
sdbusplus::async::task<bool> LatticeXO5DCPLD::eraseNonActiveImage()
{
    ...
    uint8_t* pKeyBlobOffset = pNormalKeyBlob;
...
sdbusplus::async::task<bool> LatticeXO5DCPLD::programCustomerImage()
{
    ...
    uint8_t* pRemainImageOffset = pCustomerImage;
    uint16_t fragmentFlag;
    uint16_t statusLength;
    uint16_t retryCnt = 0;
    uint8_t dataLength;
