...
sdbusplus::async::task<> EEPROMDevice::processOsStateChange()
{
    ...
    const auto expectedOsState =
        State::operating_system::Status::convertOSStatusToString(
            *requiredOsState);

    auto handleOsState =
        [&](const std::string& currentOsState) -> sdbusplus::async::task<void> {
        if (currentOsState != expectedOsState)
        {
            co_return;
        }

        debug("OS state {STATE} matches to retrieve the version", "STATE",
              currentOsState);
    ...
    /* Check the current OperatingSystemState before subscribing to property
     * changes. This avoids missing an earlier state transition and ensures
     * firmware version retrieval is not skipped.
     */
    try
    {
        auto osState = co_await hostPower.getOsState(ctx);
        co_await handleOsState(
            State::operating_system::Status::convertOSStatusToString(osState));
    }
    catch (const std::exception& e)
    {
        debug("Failed to get OperatingSystemState: {ERR}", "ERR", e.what());
    }

    while (!ctx.stop_requested())
    {
        auto nextResult = co_await hostPower.osStateChangedMatch.next<
            std::string, std::map<std::string, std::variant<std::string>>>();

        const auto& [interfaceName, changedProperties] = nextResult;

        auto it = changedProperties.find("OperatingSystemState");
        if (it != changedProperties.end())
        {
            const auto& currentOsState = std::get<std::string>(it->second);
            co_await handleOsState(currentOsState);
        }
    }
...
sdbusplus::async::task<> EEPROMDevice::processHostStateChange()
{
    ...
    /* Check the current HostState before subscribing to property changes.
     * This avoids missing an earlier state transition and ensures firmware
     * version retrieval is not skipped.
     */
    try
    {
