...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    if (!eepromFile)
    {
        lg2::error("Failed to open EEPROM file: {PATH}", "PATH", eepromPath);
        co_return false;
    }

    lg2::debug("[EEPROM] writing EEPROM device, image size: {SIZE}", "SIZE",
               image_size);

    if (image_size >
        static_cast<size_t>(std::numeric_limits<std::streamsize>::max()))
    {
        lg2::error("Image size {SIZE} exceeds streamsize max limit", "SIZE",
                   image_size);
        co_return false;
    }
    ...
    if (!eepromFile)
    {
        lg2::error("Failed to write data to EEPROM file");
        eepromFile.close();
        co_return false;
    }
