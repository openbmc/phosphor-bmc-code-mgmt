...
public:
	...
	// of the currently running software, which provides it access to the old software id.
	// This function should start the timer to run the async update function.
	virtual void startUpdate(sdbusplus::message::unix_fd image, sdbusplus::common::xyz::openbmc_project::software::ApplyTime::RequestedApplyTimes applyTime, const std::string& oldSwId) = 0;
...
public:
	CodeUpdater(const std::shared_ptr<sdbusplus::asio::connection>& bus, boost::asio::io_context& io, const std::string& serviceName, bool dryRun);

	void startUpdate(sdbusplus::message::unix_fd image, sdbusplus::common::xyz::openbmc_project::software::ApplyTime::RequestedApplyTimes applyTime, const std::string& oldSwId) final;
	...
	const std::string serviceName = "xyz.openbmc_project.Software.FWUpdate.Undefined";
