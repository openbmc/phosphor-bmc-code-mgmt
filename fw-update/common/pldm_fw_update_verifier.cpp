...
static uint8_t* readPLDMPackage(FILE* file, const size_t package_size)
{
    // Allocate memory to read the package
    uint8_t* package_data = (uint8_t*)malloc(package_size);
    if (!package_data)
    {
...
int verifyPLDMPackage(sdbusplus::message::unix_fd image)
{
    ...
    uint8_t* package_data = readPLDMPackage(file, package_size);
