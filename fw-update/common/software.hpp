...
{
  public:
    Software(const std::shared_ptr<sdbusplus::asio::connection>& conn,
             const char* objPath, const std::string& softwareId,
             const bool isDryRun) :
        swid(softwareId), dryRun(isDryRun), versionIntf(*conn, objPath),
        activationIntf(*conn, objPath), assocIntf(*conn, objPath)
    {
        lg2::info(
