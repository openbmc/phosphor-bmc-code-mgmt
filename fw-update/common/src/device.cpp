...
Device::Device(sdbusplus::async::context& io, bool isDryRun,
               ...
               const std::string& objPathConfig, SoftwareManager* parent) :
    objPathConfig(objPathConfig), parent(parent), dryRun(isDryRun),
    bus(io.get_bus()), io(io), vendorIANA(vendorIANA), compatible(compatible)
{}
...
// NOLINTEND
{
    ...
    bool dryRun = false;
    // create the new instance of the Software class
    this->softwareUpdate =
        std::make_unique<Software>(io, objPathConfig.c_str(), dryRun, *this);

    // this deletes the old sw also
    const bool success = co_await Device::continueUpdate(image, applyTime);
    ...
    if (success)
    {
        ...
        this->softwareCurrent = std::move(this->softwareUpdate);
        this->softwareUpdate = nullptr;
...
int Device::getUpdateTimerDelaySeconds()
{
    return 5;
}
                         ...
                         const std::string& oldSwId)
{
    ...
    lg2::debug("new fd after dup(fd): {FD}", "FD", this->image.fd);

    this->applyTime = applyTime;
    this->oldSwId = oldSwId;

    lg2::debug("starting timer for async update in {SECONDS}s\n", "SECONDS",
               this->getUpdateTimerDelaySeconds());

    // NOLINTBEGIN
    io.spawn([](sdbusplus::async::context& io,
                Device* device) -> sdbusplus::async::task<> {
        co_await sdbusplus::async::sleep_for(
            io, std::chrono::seconds(device->getUpdateTimerDelaySeconds()));
...
sdbusplus::async::task<bool>
    Device::continueUpdate(sdbusplus::message::unix_fd image,
                           sdbusplus::common::xyz::openbmc_project::software::
                               ApplyTime::RequestedApplyTimes applyTime)
// NOLINTEND
{
    ...
    const bool success = co_await continueUpdateWithMappedPackage(
        pldm_pkg, pldm_pkg_size, applyTime);
...
// NOLINTEND
{
    ...
    status = pldmutil::extractMatchingComponentImage(
        pp, compatible, vendorIANA, &matchingComponentOffset,
    ...
    softwareUpdate->optSoftwareActivationProgress =
        std::make_unique<SoftwareActivationProgress>(io, objPath.c_str());
    ...
    if (applyTime == applyTimeImmediate)
    {
        ...
        // We cannot implement that right now since the inventory item is
        // unclear.
        // We currently associate to the configuration item, which is always the
        // same.

        // (design): Create Interface xyz.openbmc_project.Software.Update at
        softwareUpdate->enableUpdate();
