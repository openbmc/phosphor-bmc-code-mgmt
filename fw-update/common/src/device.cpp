...
// NOLINTEND
{
    ...
    std::unique_ptr<Software> softwareUpdate =
        std::make_unique<Software>(ctx, config.objPathConfig.c_str(), *this);
    ...
    const bool success =
        co_await Device::continueUpdate(image, applyTime, softwareUpdate);
...
// NOLINTEND
{
    ...
    const bool success = co_await continueUpdateWithMappedPackage(
        pldm_pkg, pldm_pkg_size, applyTime, softwareUpdate);
