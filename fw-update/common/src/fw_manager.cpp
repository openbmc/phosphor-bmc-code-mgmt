...
FWManager::FWManager(sdbusplus::async::context& io,
                     const std::string& configType, bool isDryRun, bool debug) :
    dryRun(isDryRun), debug(debug), configType(configType), bus(io.get_bus()),
    io(io), manager(io, "/")
{
    lg2::debug("initialized FWManager with configuration type {TYPE}", "TYPE",
               configType);
...
std::string FWManager::getConfigurationInterface()
{
    return "xyz.openbmc_project.Configuration." + this->configType;
}

sdbusplus::async::context& FWManager::getIOContext()
{
    return io;
}
