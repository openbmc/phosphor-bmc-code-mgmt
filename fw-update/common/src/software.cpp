...
Software::Software(sdbusplus::async::context& ctx, const char* objPathInventory,
                   const std::string& version, Device& parent) :
    Software(ctx, objPathInventory, Software::getRandomSoftwareId(parent),
             version, parent)
{}

Software::Software(sdbusplus::async::context& ctx, const char* objPathInventory,
                   const std::string& softwareId, const std::string& version,
                   Device& parent) :
    Software(ctx, Software::getObjPathFromSwid(softwareId), objPathInventory,
             softwareId, version, parent)
{}

Software::Software(sdbusplus::async::context& ctx, std::string objPath,
                   const char* objPathInventory, const std::string& softwareId,
                   const std::string& version, Device& parent) :
...
{
    lg2::debug("{SWID}: creating dbus interfaces on path {OBJPATH}", "SWID",
               softwareId, "OBJPATH", objPath);

    std::string forward = "inventory";
    std::string reverse = "software";
    std::string endpoint = std::string(objPathInventory);

    lg2::debug("{SWID}: setting up association to {OBJPATH}", "SWID",
               softwareId, "OBJPATH", endpoint);
    this->associations({{forward, reverse, endpoint}});

    lg2::debug("{SWID}: setting version to {VERSION}", "SWID", softwareId,
               "VERSION", version);
