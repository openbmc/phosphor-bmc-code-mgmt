...
Software::Software(sdbusplus::async::context& ctx, const char* objPathConfig,
    ...
    Software(ctx, objPathConfig, Software::getRandomSoftwareId(parent),
             isDryRun, parent)
{}
...
Software::Software(sdbusplus::async::context& ctx, std::string objPath,
    ...
    swid(softwareId), dryRun(isDryRun), parent(parent), ctx(ctx)
{
...
void Software::enableUpdate()
{
    ...
    optSoftwareUpdate =
        std::make_unique<SoftwareUpdate>(this->ctx, objPath.c_str(), *this);
}
