...
std::set<std::string> SoftwareManager::getConfigurationInterfaces()
{
    std::set<std::string> res;
    for (const std::string& configType : this->getEMConfigTypes())
    {
        res.insert("xyz.openbmc_project.Configuration." + configType);
    }
    return res;
}
...
{
    // TODO(alex): cleanup here, perhaps move the mux gpio config out of the
    // common code?
    auto configInterfaces = this->getConfigurationInterfaces();
    for (auto& iface : configInterfaces)
    {
        return iface + ".MuxGpios";
    }
    throw std::invalid_argument("no configuration interfaces");
}
...
// NOLINTEND
{
    ...
    for (auto& iface : this->getConfigurationInterfaces())
    {
    ...
    for (auto& [path, v] : res)
    {
        for (auto& [service, interfaceNames] : v)
        {
            ...
            for (std::string& interfaceName : interfaceNames)
            {
                ...
                for (auto& iface : this->getConfigurationInterfaces())
                {
                    if (interfaceName.starts_with(iface))
                    {
                        found = true;
