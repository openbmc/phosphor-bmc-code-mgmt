    ...
    auto applyTime) -> sdbusplus::async::task<start_update_t::return_type>
{
    ...
    // sdbusplus::message::unix_fd imageDup = dup(image.fd);
    ...
    // NOLINTBEGIN
    ctx.spawn([](Device& device, int imageDup, RequestedApplyTimes applyTime,
                 const std::string newSwid) -> sdbusplus::async::task<> {
        co_await device.startUpdateAsync(imageDup, applyTime, newSwid);
        co_return;
    }(device, imageDup, applyTime, newSwid));
    // NOLINTEND
