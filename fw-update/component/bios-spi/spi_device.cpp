...
SPIDevice::SPIDevice(sdbusplus::async::context& io,
                     const std::string& serviceName, bool dryRun, bool hasME,
                     const std::vector<std::string>& gpioLines,
                     const std::vector<uint8_t>& gpioValues,
                     FWManager* parent) :
    Device(io, serviceName, dryRun, parent), hasManagementEngine(hasME),
                            ...
                            const std::string& oldSwId)
{
    ...
    */
    ...
    updateTimer = std::make_shared<sdbusplus::Timer>(events, [this]() {
