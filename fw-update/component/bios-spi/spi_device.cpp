...
bool SPIDevice::isSPIFlashBound()
{
    ...
    lg2::debug("checking {PATH}", "PATH", path);
...
// NOLINTEND
{
    ...
    success = co_await parent->setHostPowerstate(prevPowerstate);
    if (!success)
    {
        lg2::error("error changing host power state");
        co_return false;
...
// NOLINTEND
{
    // lg2::debug("[debug] sleep for 60 seconds");
    // co_await sdbusplus::async::sleep_for(io, std::chrono::seconds(60));
