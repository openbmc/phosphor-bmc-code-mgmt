    ...
    FWManager(io, "BIOS", isDryRun)
{
    ...
    std::shared_ptr<BiosSW> bsws =
        std::make_shared<BiosSW>(bus, swid, objPath, this);
...
void SPIDeviceCodeUpdater::startUpdateAsync()
{
                                         ...
                                         const std::string& lineName)
{
    ...
    const gpiod::line::offset offset = chip.get_line_offset_from_name(lineName);
    ...
    s.set_active_low(false);
    s.set_output_value(gpiod::line::value::ACTIVE);

    rb.add_line_settings(offset, s);
    ...
    gpiod::line_request rq = rb.do_request();

    if (!rq)
    {
        lg2::error("error making the gpio line request");
        FWManager::setHostPowerstate(true);
        return;
    }
...
// const auto actActive =
// sdbusplus::common::xyz::openbmc_project::software::Activation::Activations::Active;
...
void SPIDeviceCodeUpdater::continueUpdate(
