    ...
    const std::string& service, const std::string& path, const size_t ngpios)
{
    ...
    const uint32_t vendorIANA = std::stoi(optVendorIANA.value());

    auto spiDevice =
        std::make_shared<SPIDevice>(io, dryRun, hasME, gpioLines, gpioValues,
                                    vendorIANA, optCompatible.value(), this);
