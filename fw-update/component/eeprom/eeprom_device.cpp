...
EEPROMDevice::EEPROMDevice(
    sdbusplus::async::context& ctx, bool dryRun, const uint8_t& bus,
    const uint8_t& address, const std::vector<MuxGpioConfig>& muxGpioConfigs,
    DeviceConfig& config, SoftwareManager* parent) :
    Device(ctx, dryRun, config, parent),
    busNo(bus), address(address), muxGpioConfigs(muxGpioConfigs)
{
...
std::string EEPROMDevice::getI2CDeviceId()
{
    ...
    oss << busNo << "-" << std::hex << std::setfill('0') 
        << std::setw(4) << static_cast<int>(address);
    return oss.str();
}
...
sdbusplus::async::task<bool> EEPROMDevice::writeEEPROM(
    const uint8_t* image, size_t image_size)
{
    bool success = true;
    ...
    for (const auto& muxGpioConfig : muxGpioConfigs)
    {
        ...
        if (line.is_used())
        {
            lg2::error("gpio line {LINE} was still used", "LINE", 
                       line.name());
            co_return false;
        }
    ...
    const uint8_t* image, size_t image_size)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (!eepromFile)
            {
                lg2::error("Failed to open EEPROM file: {PATH}", 
                           "PATH", eepromPath);
                success = false;
            }
            else
