...
std::unique_ptr<VR::VoltageRegulator> XDPE152XX::create(i2c::I2CInterface* inf)
{
    std::unique_ptr<XDPE152XX> dev(new XDPE152XX(inf));
...
int XDPE152XX::cache_crc(char* key, char* sum_str, uint32_t* sum)
{
	...
	// print(stdout,"Infineon {0}, Remaining Writes: {1}", sum, remain);
	// kv_set(key, sum, 0,0); // What is this?

	return 0;
}

int XDPE152XX::calc_crc(const uint32_t* data, int len)
{
...
int XDPE152XX::get_crc(uint32_t* sum)
{
    ...
    if (this->mfr_fw(GET_CRC, tbuf, rbuf) < 0)
    {
        // lg2::error();
        return -1;
    }
...
int XDPE152XX::programm(struct xdpe152XX_config* config, bool force)
{
	...
	for (i = 0; i < config->sect_cnt; i++)
	{
    	...
    	if ((i <= 0) || (sect->type != config->section[i-1].type))
		{
      		...
      		tbuf[1] = 0x01;
      		if ((ret = VoltageRegulator::send_receive(tbuf, 2, rbuf, 0)) < 0)
			{
