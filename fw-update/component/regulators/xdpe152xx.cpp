...
std::unique_ptr<VR::VoltageRegulator> XDPE152XX::create(std::unique_ptr<i2c::I2CInterface> inf)
{
    std::unique_ptr<XDPE152XX> dev = std::make_unique<XDPE152XX>(std::move(inf));
...
int XDPE152XX::get_cache_crc(const char* sum_str, uint32_t* sum)
{
    ...
    lg2::info("Infineon [SUM], Remaining Writes: [REAMIN]","SUM", sum, "REMAIN", remain);
    return 0;
}
...
int XDPE152XX::get_crc(uint32_t *sum)
{
    ...
    if (this->mfr_fw(GET_CRC,static_cast<uint8_t*>(tbuf), static_cast<uint8_t*>(rbuf)) < 0)
    {
        return -1;
    }
...
int XDPE152XX::get_device_id(uint8_t* devID)
{
	...
	try
	{
       this->inf->processCall(addr, 0, static_cast<uint8_t*>(tbuf), rSize, static_cast<uint8_t*>(rbuf));
	}
...
int XDPE152XX::mfr_fw(uint8_t code, const uint8_t* data, uint8_t* resp)
{
	...
	if (data)
	{
		try
		{
            this->inf->processCall(addr ,4 , data, rSize, static_cast<uint8_t*>(rbuf));
		}
		catch(i2c::I2CException &e)
		{
	...
	try
	{
		this->inf->processCall(addr, code, static_cast<uint8_t*>(tbuf), rSize, static_cast<uint8_t*>(rbuf));
	}
	catch(i2c::I2CException &e)
	{
		lg2::error("mfr_fw failed: {ERR}", "ERR", e.what());
...
int XDPE152XX::lock_reg()
{
    ...
    try
	{
		this->inf->processCall(addr, tSize, static_cast<uint8_t*>(tbuf), rSize, static_cast<uint8_t*>(rbuf));
	}
	catch(i2c::I2CException &e)
	{
...
int XDPE152XX::programm(struct xdpe152XX_config* config, bool force)
{
	...
	if (!force && (remain <= VR::VR_WARN_REMAIN_WR))
	{
    	printf("WARNING: the remaining writes is below the threshold value %d!\n",
          VR::VR_WARN_REMAIN_WR);
    	printf("Please use \"--force\" option to try again.\n");
    	//syslog(LOG_WARNING, "% insufficient remaining writes %u", __func__, remain);
    	return -1;
	}
...
std::unique_ptr<VoltageRegulator> XDPE152XX::create(std::unique_ptr<i2c::I2CInterface> inf)
{
    std::unique_ptr<XDPE152XX> vr(new XDPE152XX(std::move(inf)));
    return vr;
}
