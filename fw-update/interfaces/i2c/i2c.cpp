...
namespace i2c
{
    ...
    void I2C::open()
    {
        ...
        if (ret < 0)
        {
            ...
            throw I2CException("Failed to set I2C_SLAVE", busStr, device_address, errno);
        }

    }

    void I2C::send_receive(uint8_t addr, uint8_t* writeData, uint8_t writeSize,
            uint8_t* readData, uint8_t& readSize) const
    {
        ...
        std::memcpy(readData, &(readbuf[1]), readSize);
    }
