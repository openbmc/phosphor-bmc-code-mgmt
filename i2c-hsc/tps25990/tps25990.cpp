    ...
    while (std::getline(imageStream, line))
    {
        ...
        if (pos != std::string::npos)
        {
            try
            {
                ...
                std::string valueStr = std::string(line.substr(pos + delimiter.size()));
                size_t idx = 0;
                ...
                {
                    error("parseImage failed. Invalid data length. Parsed length: {LENGTH}", 
            ...
            {
                error("parseImage failed. Exception: {ERROR}", "ERROR", e.what());
                co_return false;
            }
        }
    }
