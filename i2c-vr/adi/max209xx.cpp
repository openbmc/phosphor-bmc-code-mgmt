...
// One decoded Intel HEX record: ":llaaaatt[dd...]cc". See
// https://developer.arm.com/documentation/ka003292/latest/ for more
// details on the Intel HEX format.
...
std::optional<IntelHexRecord> parseIntelHexLine(std::string_view line)
{
    ...
    static constexpr size_t minHexChars = 10;
    ...
    static constexpr size_t byteCountIdx = 0;
    static constexpr size_t addrHiIdx = 1;
    static constexpr size_t addrLoIdx = 2;
    static constexpr size_t typeIdx = 3;
    static constexpr size_t dataStartIdx = 4;
    ...
    uint8_t byteCount = bytes[byteCountIdx];
...
// Returns the next line in `content` starting at `pos`, with a trailing
// CR stripped, and advances `pos` past the line (and its newline, if any).
std::string_view nextLine(std::string_view content, size_t& pos)
{
    auto newlinePos = content.find('\n', pos);
    std::string_view line = (newlinePos == std::string_view::npos)
                                ? content.substr(pos)
                                : content.substr(pos, newlinePos - pos);
    pos = (newlinePos == std::string_view::npos) ? content.size()
                                                   : newlinePos + 1;
...
void MAX209XX::dumpRailRegisters(std::string_view rail,
                                                   ...
                                                   size_t imageSize)
{
    ...
    {
        std::string_view line = nextLine(content, pos);
