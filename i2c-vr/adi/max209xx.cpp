...
std::optional<IntelHexRecord> parseIntelHexLine(std::string_view line)
{
    ...
    {
        uint8_t byte = 0;
        auto [ptr, ec] = std::from_chars(line.data() + i, line.data() + i + 2,
                                         byte, 16);
        if (ec != std::errc())
        {
            return std::nullopt;
                                                    ...
                                                    size_t imageSize)
{
    ...
    debug(
        "MAX209XX: verify summary: Rail A registers={COUNTA} "
...
{
    std::vector<uint8_t> tbuf = buildByteVector(PMBusCmd::writeProtect,
                                                uint8_t(0x00));
                                                       ...
                                                       uint8_t expected)
{
    ...
    {
        error(
            "MAX209XX: Scenario_Control (0xFB[7:4]) mismatch after "
...
sdbusplus::async::task<bool> MAX209XX::getCRC(uint32_t* checksum)
{
    ...
    auto crcBytes = std::span(rbuf).subspan(1 + crcBlockDataLen - crcByteLen,
                                            crcByteLen);
