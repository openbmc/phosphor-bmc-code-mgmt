...
class MAX209XX : public VoltageRegulator
{
  ...
  private:
    ...
    /**
     * @brief All register writes belonging to a single rail (Rail A / Rail
     *        B) section of a MAX209xx PMBus register configuration file.
     */
    struct MAX209xxRailConfig
    {
        std::vector<MAX209xxReg> registers;

        // Target value of the Scenario_Control register (0xFB), i.e. the
        // value this rail's register list writes to 0xFB, if present.
    ...
    sdbusplus::async::task<bool> verifyScenario(uint8_t page, uint8_t expected);
    static void dumpRailRegisters(std::string_view rail,
