...
{
    auto client = SDBusAsync::proxy().service(service).path(path).interface(
...
SDBusAsync::task<bool> I2CVRSoftwareManager::initDevice(
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    auto i2cDevice = std::make_unique<I2CDevice::I2CVRDevice>(
        ctx, vrType, static_cast<uint8_t>(busNum.value()),
        static_cast<uint8_t>(address.value()), config, this);
