    ...
    for (size_t i = 0; i < imageSize; i++)
    {
        if (image[i] == '\n') // We have a hex file, so we check new line.
        {
            ...
            else if (sepLine[0] == RecordTypeData)
            {
                ...
                configuration.pData[dcnt].len = sepLine[1] - 3;
                configuration.pData[dcnt].pec =
                    sepLine[3 + configuration.pData[dcnt].len + 1];
                configuration.pData[dcnt].addr = sepLine[2];
                configuration.pData[dcnt].cmd = sepLine[3];
                std::memcpy(configuration.pData[dcnt].data, sepLine + 4,
                            configuration.pData[dcnt].len);
                switch (dcnt)
                {
                    case RAACfgId:
                    ...
                    case RAAGen3LegacyCRC:
                        ...
                        {
                            error("unplausible CRC type check");
                        }
                        break;
