...
bool ISL69269::parseImage(const uint8_t* image, size_t imageSize)
{
    ...
    for (size_t i = 0; i < imageSize; i++)
    {
        ...
        if (image[i] == '\n') // We have a hex file, so we check new line.
        {
            ...
            int k = 0;
            size_t j = 0;
            info("Iterating line {LINENO}","LINENO", dcnt);
            for (k = 0, j = 0; j < lineLen; k++, j += 2)
            {
                info("Copy into xdigit");
        ...
        info("loop end with image index {INDEX}","INDEX", i);
    }
...
void ISL69269::parseConfigLine(const uint32_t* line, raaData* entry)
{
    entry->len = line[1] - 2;
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    uint32_t devCRC = 0;
    // NOLINTBEGIN(clang-analyzer-core.uninitialized.Branch)
    if (!(co_await this->getCRC(&devCRC)))
    // NOLINTEND(clang-analyzer-core.uninitialized.Branch)
    {
        error("program failed at getCRC");
        co_return false;
