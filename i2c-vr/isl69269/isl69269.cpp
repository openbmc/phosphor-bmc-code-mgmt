...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    ret = co_await this->i2cInterface.sendReceive(tbuf, 1, rbuf, RAADeviceRevisionLen+1);
    if (!ret)
    {
    ...
    for (size_t i = 0; i < imageSize; i++)
    {
        if (image[i] == '\n') // We have a hex file, so we check new line.
        {
            ...
            for (k = 0, j = 0; j < lineLen; k++, j+=2)
            {
                ...
                sepLine[k] = std::strtol(xdigit, NULL, 2);
            }
            ...
            if (sepLine[0] == RecordTypeHeader)
            {
                ...
                else if (sepLine[3] == PMBusDeviceRev)
                {
                    configuration.devRevExp =
                        ...
                        (static_cast<uint32_t>(sepLine[6]));

                    if ((configuration.devRevExp & 0xFF) < RAAGen3SWRevMin)
                    {
            ...
            {
                if ((sepLine[1]+2 >= sizeof(sepLine)))
                {
                    dcnt = 0;
                    break;
                }
                parseConfigLine(sepLine, &configuration.pData[dcnt]);
                ...
                case RAAGen3LegacyCRC:
                    if (configuration.mode == RAAGen3Legacy) {
                        std::memcpy(&configuration.crcExp, &sepLine[4], RAAChecksumLen);
                    }
                    break;
...
bool ISL69269::checkImage()
{
    ...
    {
        crc8 = calcCRC8(configuration.pData[i].data, configuration.pData[i].len+1);
...
void ISL69269::parseConfigLine(const uint8_t* line, raaData* entry)
{
    entry->len = line[1] - 2;
    entry->pec = line[3 + entry->len];
    entry->addr = line[2];
    entry->cmd = line[3];
    std::memcpy(entry->data, line+2, entry->len+1);
}

// NOLINTBEGIN(readability-static-accessed-through-instance)
sdbusplus::async::task<bool> ISL69269::program(bool _)
// NOLINTEND(readability-static-accessed-through-instance)
{
    bool ret = false;
    uint8_t tbuf[32] = {0};
    ...
    for ( int i = 0; i < configuration.wrCnt; i++)
    {
        ...
        std::memcpy(tbuf+1, &configuration.pData[i].data, configuration.pData[i].len);

        ret = co_await this->i2cInterface.sendReceive(tbuf, configuration.pData[i].len, rbuf, 0);
        if (!ret)
        {
...
// NOLINTBEGIN(readability-static-accessed-through-instance)
sdbusplus::async::task<bool> ISL69269::restoreCfg(uint8_t cfgId)
// NOLINTEND(readability-static-accessed-through-instance)
{
    bool ret = false;
    uint8_t tbuf[8] = {0};
...
// NOLINTBEGIN(readability-static-accessed-through-instance)
sdbusplus::async::task<bool> ISL69269::verifyImage(const uint8_t* image, size_t imageSize)
// NOLINTEND(readability-static-accessed-through-instance)
{
    bool ret = false;
    uint8_t mode = 0xFF;
    ...
    ret = co_await this->getHexMode(&mode);
    if (!ret)
    {
    ...
    {
        error("program failed with mode of device and configuration are not equal");
        co_return false;
    }

    ret = co_await this->getCRC(&crc);
    if (!ret)
    {
    ...
    {
        error("program failed with not matching device id of device and config");
        co_return false;
    }

    ret = co_await this->getDeviceRevision(&devRev);
    if (!ret)
    {
    ...
    {
        error("program failed with not matching revision of device and configuration");
        co_return false;
    }
...
{
    bool ret = false;
    ret = co_await this->getProgStatus();
    if (!ret)
    {
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    ret = co_await this->program(force);
    if (!ret)
    {
