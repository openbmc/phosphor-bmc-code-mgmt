...
// NOLINTEND(readability-static-accessed-through-instance)
{
    bool ret = false;
    uint32_t devCRC = 0;
    ret = co_await this->getCRC(&devCRC);
    if (!ret)
    {
        error("program failed at getCRC");
        co_return false;
    ...
    if (!force && (configuration.crcExp == devCRC))
    {
        error("update failed - same CRC requires force");
        co_return false;
    }

    ret = co_await this->program();
    if (!ret)
    {
