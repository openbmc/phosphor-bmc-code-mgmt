...
constexpr uint8_t programmBufferSize = 32;
...
{
    uint8_t tbuf[programmBufferSize] = {0};
    uint8_t rbuf[programmBufferSize] = {0};
    uint8_t rlen = zeroByteLen;
...
sdbusplus::async::task<bool> ISL69269::getProgStatus()
{
    ...
    do
    {
        // NOTLINTBEGIN(clang-analyzer-core.uninitialized.Branch)
        if (!(co_await dmaReadWrite(tbuf, rbuf)))
        // NOLINTEND(clang-analyzer-core.uninitialized.Branch)
        {
