        ...
        if (image[i] == '\n') // We have a hex file, so we check new line.
        {
            ...
            if (sepLine[0] == recordTypeHeader)
            {
                ...
                else if (sepLine[3] == pmBusDeviceRev)
                {
                    ...
                    if (configuration.mode == 0) // The MSB device revision of GEN3p5 will be between 0x03 and 0x06 (inclusive), so it cannot be used alone to determine Gen3 or GEN3p5.
                    {
                                                   ...
                                                   size_t imageSize)
{
    ...
    switch (mode)
    {
        ...
        case gen3p5:
            ...
            {
                error(
                    "revision requirements for Gen3p5 device not fulfilled");
            }
            break;
    }
