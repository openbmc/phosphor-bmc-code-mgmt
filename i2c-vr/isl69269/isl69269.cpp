...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    if (!(co_await this->i2cInterface.sendReceive(tbuf, tLen, rbuf, rLen)))
    {
        error("getDeviceId failed");
        co_return false;
    }

    std::memcpy(deviceId, &rbuf[1], RAADeviceIdLength);
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    {
        error(
            "program failed with unmatching revisions. Want: {WANT}, Have: {HAVE}",
            "WANT", lg2::hex, devRev, "HAVE", lg2::hex,
            configuration.devRevExp);
        co_return false;
    }
