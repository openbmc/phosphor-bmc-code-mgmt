...
namespace phosphor::software::VR
{
    ...
    class ISL69269 : public VoltageRegulator
    {
        ...
        private:
            ...
            sdbusplus::async::task<bool> getProgStatus(int* status);
            sdbusplus::async::task<bool> restoreCfg(uint8_t id);

            bool parseImage(const uint8_t* image, size_t imageSize);
            bool checkImage();
            static uint8_t calcCRC8(const uint8_t* data, uint8_t len);
            static void parseConfigLine(const uint8_t* line, raaData* entry);
