...
static constexpr std::string_view mp297xDriverPath =
    "/sys/bus/i2c/drivers/mp2975";
...
sdbusplus::async::task<bool> MP297X::checkMTPCRC(uint32_t expectedCRC)
{
    uint32_t crc = 0;
...
sdbusplus::async::task<bool> MP297X::reset()
{
    // MPS not support reset command, so we just return true
    co_return true;
}

sdbusplus::async::task<bool> MP297X::setDriver(bool bind)
{
    std::ostringstream devName;
    devName << std::setw(2) << std::setfill('0') << std::dec << bus << "-"
            << std::setw(4) << std::setfill('0') << std::hex
            << static_cast<int>(address);

    auto path = std::string(mp297xDriverPath) + (bind ? "/bind" : "/unbind");

    std::ofstream os(path);
    if (!os)
    {
        error("Failed to open {PATH}", "PATH", path);
        co_return false;
    }
    os << devName.str();
    os.close();

    //  wait kernel to complete the bind/unbind
    co_await sdbusplus::async::sleep_for(ctx, std::chrono::seconds(1));

    auto devPath = std::string(mp297xDriverPath) + "/" + devName.str();
    auto exists = std::filesystem::exists(devPath);

    if ((bind && !exists) || (!bind && exists))
    {
        error("Failed to {ACTION} device {DEV_NAME}", "ACTION",
              bind ? "bind" : "unbind", "DEV_NAME", devName.str());
        co_return false;
    }

    debug("{RESULT}: device {DEV_NAME}", "RESULT", bind ? "Bound" : "Unbound",
          "DEV_NAME", devName.str());

    co_return true;
}

sdbusplus::async::task<bool> MP297X::update()
{
    ...
    if (!(co_await checkMTPCRC(expectedCRC)))
    {
        co_return false;
    }
...
{
    (void)force;
    // Unbind the kernel driver before programming.
    // Firmware update requires strict sequential writes to MTP cells,
    // with no sectors skipped. During Multi-Config Page programming,
    // I2C reads are not allowed until the entire page2A write is done.
    // Leaving the driver bound could cause reads and corrupt the update.
    if (!co_await setDriver(false))
    {
        co_return false;
    }
    ...
    status = status && co_await setDriver(true);
