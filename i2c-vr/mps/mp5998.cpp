...
static constexpr std::string_view mp5998DriverPath = 
    "/sys/bus/i2c/drivers/mp5998";
...
sdbusplus::async::task<bool> MP5998::programAllRegisters()
{
    ...
    for (const auto& regData : configuration->registersData)
    {
        ...
        {
            std::vector<uint8_t> tbuf = buildByteVector(PMBusCmd::page, 
                                                        regData.page);
            std::vector<uint8_t> rbuf;
            if (!i2cInterface.sendReceive(tbuf, rbuf))
            {
        ...
        if (!i2cInterface.sendReceive(tbuf, rbuf))
        {
            error("Failed to write register 0x{REG} on page {PAGE}", 
                  "REG", lg2::hex, regData.addr, "PAGE", regData.page);
            co_return false;
        }
    }
...
sdbusplus::async::task<bool> MP5998::waitForMTPComplete()
{
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(mtpStoreWaitmS));
...
sdbusplus::async::task<bool> MP5998::restoreMTPAndVerify()
{
    ...
    std::this_thread::sleep_for(std::chrono::microseconds(mtpRestoreWait));
