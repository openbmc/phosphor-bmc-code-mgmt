...
class MP994XImageParser : public MPSImageParser
{
  public:
    MP994XImageParser(const uint8_t* image, size_t imageSize) :
        MPSImageParser(image, imageSize)
    {}

    MPSData extractData(const std::vector<std::string_view>& tokens) override
    {
        MPSData data;
...
sdbusplus::async::task<bool> MP994X::verifyImage(const uint8_t* image,
                                                 size_t imageSize)
{
    auto parser = std::make_unique<MP994XImageParser>(image, imageSize);
    if (!co_await parseImage(image, imageSize, std::move(parser)))
    {
...
sdbusplus::async::task<bool> MP994X::setMultiConfigAddress(uint8_t config)
{
    ...
    // Resulting values for config set 1 ~ 8: 0x40 ~ 0x47
    auto addr = config - 1;
