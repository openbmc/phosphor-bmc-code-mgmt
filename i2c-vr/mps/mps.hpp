...
class MPSVoltageRegulator : public VoltageRegulator
{
  ...
  protected:
    /**
     * @brief Writes data to the VR using variadic arguments.
     *
     * Constructs a byte buffer from the given arguments and writes it to the
     * I2C device. Supported argument types include:
     * - Enums (converted to uint8_t)
     * - Integral types (written in little-endian order)
     * - std::initializer_list<uint8_t> (appended directly)
     *
     * If a single std::vector<uint8_t> is passed, this overload is disabled
     * to prevent recursion with the overload that directly takes a vector.
     *
     * @tparam Args Parameter pack of supported data types
     * @param args The values to be written to the VR
     ...
     *
     * This is the core I2C write function used internally by the templated
     * writeData function.
     *
    ...
    /**
     * @brief Sets the active MPS page.
     *
     * Issues a command to the device to change the active page for subsequent
     * operations.
     *
    ...
    /**
     * @brief I2C interface for communication with the device.
     */
