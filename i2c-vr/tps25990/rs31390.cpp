                                                 ...
                                                 size_t imageSize)
{
    ...
    while (std::getline(imageStream, line))
    {
        ...
        uint8_t reg = static_cast<uint8_t>(std::stoul(
            cols[static_cast<size_t>(RS31390Img::commandCode)], nullptr, 16));

        if (reg == regCheckSum)
        {
            configuration.checksum = static_cast<uint32_t>(std::stoul(
                cols[static_cast<size_t>(RS31390Img::value)], nullptr, 16));
            continue;
        }

        configuration.offsets.push_back(reg);

        uint16_t value = static_cast<uint16_t>(std::stoul(
            cols[static_cast<size_t>(RS31390Img::value)], nullptr, 16));

        if (std::stoul(cols[static_cast<size_t>(RS31390Img::sizeInBytes)],
                       nullptr, 16) == oneByte)
        {
            uint8_t lo = static_cast<uint8_t>(value & 0xFF);
            configuration.data.push_back({lo});
        }
        else if (std::stoul(cols[static_cast<size_t>(RS31390Img::sizeInBytes)],
                            nullptr, 16) == twoBytes)
        {
            uint8_t lo = static_cast<uint8_t>(value & 0xFF);
