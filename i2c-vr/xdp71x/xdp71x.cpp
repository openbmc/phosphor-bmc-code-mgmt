...
const std::vector<RegData> pmbusCfgRegs = {
    ...
    {0xE5, 2}, {0xE6, 2}, {0xE7, 2}, {0xE8, 2}, {0xE9, 2}, {0xFA, 1}};
...
sdbusplus::async::task<bool> XDP71X::getCRC(uint32_t* sum)
{
    ...
    for (auto& cfgReg : pmbusCfgRegs)
    {
        ...
        // XDP710 is byte and no need 0xFA
        if (cfgReg.reg == 0xD8 && isXDP710)
        {
            rbuf.resize(1);
        }
        else
                                                 ...
                                                 size_t imageSize)
{
    (void)image;
    (void)imageSize;
    warning("Firmware update not supported for XDP71X");
    co_return false;
}
