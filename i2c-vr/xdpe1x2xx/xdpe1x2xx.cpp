...
XDPE1X2XX::XDPE1X2XX(SDBusAsync::context& ctx, uint16_t bus, uint16_t address) :
...
int XDPE1X2XX::mfrFWcmd(uint8_t cmd, uint8_t* data, uint8_t* resp)
{
    ...
    if (data)
    {
        ...
        if (ret < 0)
        {
            error("Failed to send MFR command: {CMD} with {ERR}", "CMD",
                  std::string("IFXMFRFwCmdDAta"), "ERR", ret);
            return -1;
        }
    }
    ...
    {
        error("Failed to send MFR command: {CMD} with {ERR}", "CMD",
              std::string("IFXMFRFwCmd"), "ERR", ret);
        return ret;
    }
...
int XDPE1X2XX::program(bool force)
{
    ...
    {
        error(
            "Failed to program the VR - 3 or less remaining writes left and not force");
        return -1;
    }
    ...
    // calculation These values are taken from
    // https://github.com/facebook/openbmc/blob/36ea8f18399990589e4626e982a8bb844ad454fd/common/recipes-lib/vr/files/xdpe152xx.c#L355
...
SDBusAsync::task<bool> XDPE1X2XX::verifyImage(const uint8_t* image,
                                              size_t imageSize)
