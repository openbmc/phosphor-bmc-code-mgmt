...
int XDPE1X2XX::getDeviceId(uint8_t* devID)
{
    ...
    if (ret < 0)
    {
        lg2::error("XDPE1X2XX::getDeviceID() failed");
        return ret;
    }
...
int XDPE1X2XX::ctlRegs(bool lock, uint32_t password) {
    ...
    if (ret < 0)
    {
        lg2::error("XDPE1X2XX::cltRegs() failed");
        return ret;
    }
    return 0;
}
...
int XDPE1X2XX::mfrFWcmd(uint8_t cmd, uint8_t* data, uint8_t* resp)
{
    ...
    if (data)
    {
        ...
        if ( ret < 0 )
        {
            lg2::error("XDPE1X2XX::mfrFWcmd(IFX_MFR_FW_CMD_DATA) failed");
            return -1;
        }
    }
   ...
   if (ret < 0)
   {
       lg2::error("XDPE1X2XX::mfrFWcmd() failed");
       return ret;
   }
   ...
   if (resp)
   {
        ...
        if (rBuf[0] != 4)
        {
            lg2::error("XDPE1X2XX::mfrFWcmd() failed, size of response not 4");
            return -1;
        }
...
int XDPE1X2XX::getRemainingWrites(uint8_t* remain)
{
    ...
    if (ret <  0)
    {
        lg2::error("XDPE1X2XX::getRemainingWrites() failed at mfrFWcmd(MFR_FW_CMD_RMNG))");
        return ret;
    }
    ...
    if (ret < 0)
    {
        lg2::error("XDPE1X2XX::getRemainingWrites() failed at getDeviceId()");
        return ret;
    }
...
int XDPE1X2XX::getCRC(uint32_t* sum)
{
    ...
    if (ret < 0)
    {
        lg2::error("XDPE1X2XX::getCRC() failed at mfrFWcmd(MFR_FW_CMD_GET_CRC)");
        return ret;
    }
...
int XDPE1X2XX::program(struct xdpe1x2xxConfig* cfg, bool force)
{
    ...
    {
        lg2::error("XDPE1X2XX::program() failed at getCRC()");
        return -1;
    }

    if (!force && ( sum == cfg->sumExp ))
    {
        lg2::error("XDPE1X2XX::program() failed: CRC of image and on chip identical");
        return -1;
    }

    if (this->getRemainingWrites(&remain) < 0)
    {
        lg2::error("XDPE1X2XX::program() failed at getRemainingWrites()");
        return -1;
    }

    if (!remain)
    {
        lg2::error("XDPE1X2XX::program() failed: 0 remaining writes left");
        return -1;
    }

    if (!force && (remain <= VR_WARN_REMAINING))
    {
        lg2::error("3 or less remaining writes left and not force");
        return -1;
    }
    ...
    if (ret < 0 )
    {
        lg2::error("XDPE1X2XX::program() failed at mfrFWcmd(MFW_FW_CMD_OTP_FILE_INVD)");
        return ret;
    }
    ...
    if (ret < 0)
    {
        lg2::error("XDPE1X2XX::program() failed unexpectedly");
        return ret;
    }
...
int XDPE1X2XX::checkImage(struct xdpe1x2xxConfig* cfg)
{
    ...
    for (i = 0; i < cfg->sectCnt; i++)
    {
        ...
        if (sect == NULL)
        {
            lg2::error("XDPE1X2XX::checkImage() failed: sect == Null");
            return -1;
        }
        ...
        if (crc != sect->data[2])
        {
             lg2::error("XDPE1X2XX::checkImage() failed: interrim CRC does not match expected value");
             return -1;
        }
        sum += crc;
        ...
        if (crc != sect->data[sect->dataCnt-1]) {
            lg2::error("XDPE1X2XX::checkImage() failed: second interrum CRC does not match expected value");
            return -1;
        }
        sum += crc;
    }
    ...
    if (sum != cfg->sumExp) {
        lg2::error("XDPE1X2XX::checkImage(): calculated CRC does not match CRC value writte in image");
        return -1;
    }
...
int XDPE1X2XX::fwUpdate(uint8_t* image, size_t image_size, bool force)
{
    struct xdpe1x2xxConfig cfg;

    if (parseImage(&cfg, image, image_size) < 0 )
    {
        lg2::error("XDPE1X2XX::fwUpdate() failed at parseImage() Image");
        return -1;
    }

    if (checkImage(&cfg) < 0)
    {
        lg2::error("XDPE1X2XX::fwUpdate() failed at checkImage()");
        return -1;
    }
...
int XDPE1X2XX::reset()
{
    ...
    {
        lg2::error("XDPE1X2XX::reset() failed");
        return 1;
    }
