...
bool Signature::systemLevelVerify(const std::string &name) {
  ...
  for (const auto &keyType : keyTypes) {
    ...
    valid = verifyFile(file, sigFile, keyHashPair.second, hashFunc);
    if (valid) {
      break;
    }
  }
...
bool Signature::verifyFile(const fs::path &file, const fs::path &sigFile,
  ...
  auto result =
      EVP_DigestVerifyInit(rsaVerifyCtx.get(), NULL, hashPtr, NULL, pubKey);
  ...
  result = EVP_DigestVerifyFinal(
      rsaVerifyCtx.get(), reinterpret_cast<unsigned char *>(signature()), size);
