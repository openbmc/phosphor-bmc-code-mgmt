...
bool Signature::verifyFullImage()
{
    ...
    static const std::vector<std::string> fullImages = {
        "image-bmc.sig",  "image-hostfw.sig", "image-kernel.sig",
        "image-rofs.sig", "image-rwfs.sig",   "image-u-boot.sig",
        "MANIFEST.sig",   "publickey.sig"};
    ...
    for (auto& image : fullImages)
    {
        ...
        inFile.open(imagePath, std::ios_base::in);
        inFile.peek();
        if (inFile.eof())
        {
            inFile.close();
