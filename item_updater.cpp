...
void ItemUpdater::createSequencerObject(
    const std::string& sequencerPath,
    ...
    std::unique_ptr<VersionClass>& seqVersion)
{
    ...
    if (pos == std::string::npos)
    {
        error("No version id found in object path {PATH}", "PATH",
              sequencerPath);
        return;
    }
    ...
    if (version.empty())
    {
        error("Failed to read version from {VERSION_ID}: {PATH}", "PATH",
              sequencerPath, "VERSION_ID", versionId);
        return;
    }
