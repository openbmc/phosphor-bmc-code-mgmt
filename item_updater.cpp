...
void ItemUpdater::processBMCImage()
{
    ...
    checkBootVersion(
        activations.find(id)->second->redundancyPriority.get()->priority(), id);
...
void ItemUpdater::checkBootVersion(uint8_t value, const std::string& versionId)
{
    ...
    {
        log<level::ERR>(
            "Error: Wrong boot version", entry("BOOTED_VERSION=%s", versionId.c_str()),
            entry("EXPECTED_VERSION=%s", prioritySet.begin()->first.c_str()));
    }
}
