 ...
 * (v1.0), indirect (CMS memory window) image push over a pluggable
 * Transport.
 *
 * Spec:
 * https://www.opencompute.org/documents/ocp-recovery-document-1p0-final-1-pdf
 *
 * The protocol layer (Target) only ever deals in command codes and
 * payloads; the interface-specific framing (length field, PEC) belongs
 * to the Transport, because it differs per physical interface: SMBus
 * carries a 1-byte block byte count, I3C a 2-byte length plus PEC. The
 * built-in I2CTransport implements the SMBus framing:
...
/* Largest INDIRECT_DATA payload per write transaction (sized for one
 * SMBus block write) */
inline constexpr size_t chunkSizeMax = 252;

/* Largest response payload per read transaction (one SMBus block) */
inline constexpr size_t responseSizeMax = 255;
...
/* One physical interface to a recovery-capable device. The protocol
 * layer hands over a command code and its payload; the transport owns
 * the interface-specific framing (length field, PEC), so SMBus (1-byte
 * byte count), I3C (2-byte length plus PEC) and others differ only here.
 * Implementations must accept payloads up to chunkSizeMax bytes and
 * response windows up to responseSizeMax bytes. */
class Transport
{
  public:
    Transport() = default;
    ...
    /* Write cmd with payload. Returns a default-constructed (falsy)
     * error_code on success. */
    [[nodiscard]] virtual std::error_code write(
        Cmd cmd, std::span<const uint8_t> payload) = 0;

    /* Write cmd, then read its response payload into resp with the
     * framing stripped. Returns the payload length the device reported;
     * a reported length larger than resp.size() is a protocol_error.
     * Zero is a valid length - the protocol layer decides whether it is
     * acceptable for the command. */
    [[nodiscard]] virtual std::expected<size_t, std::error_code> read(
        Cmd cmd, std::span<uint8_t> resp) = 0;
};

/* SMBus block write/read framing over ioctl(I2C_RDWR) on /dev/i2c-<bus>;
 * owns the fd. */
class I2CTransport final : public Transport
{
  public:
    ...
    std::error_code write(Cmd cmd, std::span<const uint8_t> payload) override;

    std::expected<size_t, std::error_code> read(
        Cmd cmd, std::span<uint8_t> resp) override;

  private:
    I2CTransport(int fd, uint16_t addr) : fd(fd), addr(addr) {}

    /* One bus transaction: write wbuf; a non-empty rbuf requests a
     * repeated-start read into it. */
    [[nodiscard]] std::error_code transfer(std::span<const uint8_t> wbuf,
...
class Target
{
  public:
     ...
     * with maxLen up to responseSizeMax. maxLen also sizes the bus read
     * transaction, and some I2C adapters reject reads longer than their
     * quirk limit - keep the default unless the device is known to
     * report more. */
  ...
  private:
    ...
    /* Reads the response payload for cmd into resp and returns its
     * length; lengths below minLen are protocol errors. */
    [[nodiscard]] std::expected<size_t, std::error_code> commandRead(
        Cmd cmd, std::span<uint8_t> resp, size_t minLen = 1);
