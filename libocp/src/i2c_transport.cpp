...
static std::error_code transfer(int fd, uint16_t addr,
...
std::error_code I2CTransport::write(Cmd cmd, std::span<const uint8_t> payload)
{
    ...
    return transfer(fd, addr, std::span(wbuf).first(2 + payload.size()), {});
}
    ...
    Cmd cmd, std::span<uint8_t> resp)
{
    ...
    if (auto ec =
            transfer(fd, addr, wbuf, std::span(rbuf).first(1 + resp.size())))
    {
