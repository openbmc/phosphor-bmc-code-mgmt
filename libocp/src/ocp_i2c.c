                            ...
                            uint8_t* rbuf, size_t rlen)
{
    ...
    msgs[0].buf = (uint8_t*)(uintptr_t)wbuf;
...
int ocp_open(struct ocp_ctx* ctx, int bus, uint16_t addr)
{
    char path[32];
    int fd;
...
int ocp_init_transport(struct ocp_ctx* ctx, ocp_transfer_fn fn, void* user,
                       uint16_t addr)
{
    if (ctx == NULL || fn == NULL)
    {
