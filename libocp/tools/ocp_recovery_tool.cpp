...
std::string deviceStatusStr(ocp_recov_device_status status)
{
    switch (status)
    {
        case OCP_RECOV_DEVICE_STATUS_PENDING:
            return "status pending";
        case OCP_RECOV_DEVICE_STATUS_HEALTHY:
            return "device healthy";
        case OCP_RECOV_DEVICE_STATUS_ERROR:
            return "device error";
        case OCP_RECOV_DEVICE_STATUS_RECOVERY_MODE:
            return "recovery mode - ready to accept recovery image";
        case OCP_RECOV_DEVICE_STATUS_RECOVERY_PENDING:
            return "recovery pending, waiting for device/platform reset";
        case OCP_RECOV_DEVICE_STATUS_RUNNING_RECOVERY:
            return "recovery image running";
        case OCP_RECOV_DEVICE_STATUS_BOOT_FAILURE:
            return "boot failure";
        case OCP_RECOV_DEVICE_STATUS_FATAL_ERROR:
            return "fatal error";
        default:
            return "reserved";
    }
...
std::string recoveryStatusStr(ocp_recov_recovery_status status)
{
    switch (status)
    {
        case OCP_RECOV_RECOVERY_STATUS_NOT_IN_RECOVERY:
            return "not in recovery mode";
        case OCP_RECOV_RECOVERY_STATUS_AWAITING_IMAGE:
            return "awaiting recovery image";
        case OCP_RECOV_RECOVERY_STATUS_BOOTING_IMAGE:
            return "booting recovery image";
        case OCP_RECOV_RECOVERY_STATUS_SUCCESS:
            return "recovery successful";
        case OCP_RECOV_RECOVERY_STATUS_FAILED:
            return "recovery failed";
        case OCP_RECOV_RECOVERY_STATUS_AUTH_ERROR:
            return "recovery image authentication error";
        case OCP_RECOV_RECOVERY_STATUS_ENTERED_RECOVERY:
            return "error entering recovery mode";
        case OCP_RECOV_RECOVERY_STATUS_INVALID_CMS:
...
int cmdGetDeviceId(struct ocp_recov_ctx* ctx)
{
    std::vector<uint8_t> id(32);
    size_t len = 0;

    int rc = ocp_recov_get_device_id(ctx, id.data(), id.size(), &len);
    if (rc < 0)
    {
...
int cmdGetDeviceStatus(struct ocp_recov_ctx* ctx)
{
    ...
    int rc = ocp_recov_get_prot_cap(ctx, &cap);
    if (rc == 0)
    {
        std::cout << std::format(
    ...
    rc = ocp_recov_get_device_status(ctx, &status, &protErr, &reason);
    if (rc < 0)
    {
...
int cmdGetRecoveryStatus(struct ocp_recov_ctx* ctx)
{
    ...
    int rc = ocp_recov_get_recovery_status(ctx, &status, &index, &vendor);
    if (rc < 0)
    {
...
int cmdSetForceRecovery(struct ocp_recov_ctx* ctx)
{
    int rc = ocp_recov_force_recovery(ctx);
    if (rc < 0)
    {
...
int enterRecoveryMode(struct ocp_recov_ctx* ctx)
{
    ocp_recov_device_status status = OCP_RECOV_DEVICE_STATUS_PENDING;

    int rc = ocp_recov_get_device_status(ctx, &status, nullptr, nullptr);
    if (rc < 0)
    {
        std::cerr << std::format("DEVICE_STATUS read failed: {}\n",
                                 strerror(-rc));
    ...
    {
        sleepSec(1);
        rc = ocp_recov_get_device_status(ctx, &status, nullptr, nullptr);
...
int cmdPerformRecovery(struct ocp_recov_ctx* ctx,
                       const std::vector<std::string>& images)
{
    ...
    for (const auto& path : images)
    {
        ...
        if (rc == 0)
        {
            rc = ocp_recov_write_image(ctx, image.data(), image.size(),
                                       progress, &lastPct);
        }
        if (rc == 0)
        {
    ...
    for (unsigned int poll = 0; poll < OCP_RECOV_STATUS_POLL_RETRIES; poll++)
    {
        ...
        rc = ocp_recov_get_recovery_status(ctx, &status, nullptr, nullptr);
        if (rc == 0)
        {
...
int cmdGetCmsLogs(struct ocp_recov_ctx* ctx, uint8_t window,
                  std::string outfile)
{
    ...
    int rc = ocp_recov_indirect_ctrl(ctx, window, 0);
    if (rc < 0)
    {
    ...
    rc = ocp_recov_indirect_status(ctx, nullptr, nullptr, &cmsSize);
    ...
    while (total < limit)
    {
        ...
        rc = ocp_recov_indirect_data_read(ctx, buf.data(), buf.size(), &len);
        if (rc < 0)
        {
...
int main(int argc, char** argv)
{
    ...
    int bus = -1;
    uint8_t address = 0;
    uint8_t cms = OCP_RECOV_CMS_DEFAULT;
    size_t chunk = OCP_RECOV_CHUNK_SIZE_MAX;
    uint8_t window = 2;
    std::string outfile;
    ...
    app.add_option("-k,--chunk", chunk,
                   std::format("Block write payload size, 1-{} (default {})",
                               ...
                               OCP_RECOV_CHUNK_SIZE_MAX))
        ->check(CLI::Range(size_t(1), size_t(OCP_RECOV_CHUNK_SIZE_MAX)));
    ...
    int rc = ocp_recov_open(&ctx, bus, address);
    if (rc < 0)
    {
