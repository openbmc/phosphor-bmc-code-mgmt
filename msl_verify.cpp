...
int minimum_ship_level::compare(const Version& StrToCompare, const Version& MslString)
{
...
// parse Function copy Input String onto OutString in Version format {major,minor,ver}.
void minimum_ship_level::parse(const std::string& InpString, Version& OutString)
{
    ...
    {
       log<level::ERR>("Unable to parse BMC version", entry("VERSION=%s", InpString.c_str()));
       return;
    }
...
int minimum_ship_level::verify(const std::string& InpVersionStr)
{
    ...
    // Define MslVersion variable and populate in Version format {major,minor,ver} using parse function.
    Version       MslVersion = {0, 0, 0};
    parse(msl, MslVersion);

    // Define InpTestString variable and populate in Version format {major,minor,ver} using parse function.
