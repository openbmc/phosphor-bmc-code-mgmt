        ...
        size_t& written, size_t grandTotal)
{
    ...
    if (!(co_await stageComponents(target, bus, components, written,
                                   grandTotal)))
    {
        co_return BusOutcome::failed;
    }
...
sdbusplus::async::task<bool> OCPRecoveryDevice::stageComponents(
    OCPRecov::Target& target, uint16_t bus,
    const std::vector<DeviceInf::ComponentImage>& components, size_t& written,
    size_t grandTotal)
{
    size_t index = 0;
    for (const auto& component : components)
