...
class OCPRecoveryDevice : public DeviceInf::Device
{
  ...
  private:
    ...
    sdbusplus::async::task<bool> ensureRecoveryMode(struct ocp_recov_ctx* ocp,
                                                    uint16_t bus);

    // Writes one image; 'written' accumulates and maps overall progress
    // against 'grandTotal'.
    sdbusplus::async::task<bool> writeImage(
        struct ocp_recov_ctx* ocp, uint16_t bus, const uint8_t* image,
        size_t size, size_t& written, size_t grandTotal);

    sdbusplus::async::task<bool> pollRecoveryComplete(struct ocp_recov_ctx* ocp,
                                                      uint16_t bus);
