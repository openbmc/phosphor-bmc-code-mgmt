...
namespace OCPRecov = ::ocp::recovery;
...
class OCPRecoveryDevice : public DeviceInf::Device
{
  public:
    ...
    OCPRecoveryDevice(
        sdbusplus::async::context& ctx, std::vector<uint16_t> buses,
        uint16_t address, size_t chunkSize, uint8_t cms,
        uint32_t forceRecoveryTimeout, ConfigInf::SoftwareConfig& config,
  ...
  private:
    ...
    sdbusplus::async::task<bool> ensureRecoveryMode(OCPRecov::Target& target,
                                                    uint16_t bus);

    // Stages every component image into successive CMS windows starting
    // at 'cms', without activating.
    sdbusplus::async::task<bool> stageComponents(
        OCPRecov::Target& target, uint16_t bus,
        const std::vector<DeviceInf::ComponentImage>& components,
        size_t& written, size_t grandTotal);

    // Stages one component image into its CMS window (RECOVERY_CTRL +
    // INDIRECT_CTRL + chunked image write), without activating.
    sdbusplus::async::task<bool> stageComponent(
    ...
    // against 'grandTotal'.
    sdbusplus::async::task<bool> writeImage(
        OCPRecov::Target& target, uint16_t bus, std::span<const uint8_t> image,
        size_t& written, size_t grandTotal);

    // Writes one chunk with write retries and acknowledge polling;
    // 'offset' is only used for diagnostics.
    sdbusplus::async::task<bool> writeChunk(
        OCPRecov::Target& target, uint16_t bus, std::span<const uint8_t> chunk,
        size_t offset);

    sdbusplus::async::task<bool> pollRecoveryComplete(OCPRecov::Target& target,
                                                      uint16_t bus);
