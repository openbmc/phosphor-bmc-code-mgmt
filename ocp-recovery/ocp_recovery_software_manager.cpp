...
const std::vector<std::string> emConfigTypes = {"OCPRecoveryFirmware"};

static constexpr uint32_t defaultForceRecoveryTimeout = 30;

// Fetch an optional EM configuration property without the error log that
// dbusGetRequiredProperty emits for absent properties.
template <typename T>
    ...
    SoftwareConfig& config)
{
    ...
    {
        lg2::info("[config] adding bus {BUS} to the aggregated OCP recovery device",
                  "BUS", busNum.value());
    ...
    std::optional<uint64_t> memoryWindow =
        co_await dbusGetOptionalProperty<uint64_t>(
            ctx, service, path, configIface, "MemoryWindow");
    std::optional<uint64_t> forceRecoveryTimeout =
        co_await dbusGetOptionalProperty<uint64_t>(
            ctx, service, path, configIface, "ForceRecoveryTimeoutSeconds");

    const size_t chunk = std::clamp<uint64_t>(
        chunkSize.value_or(OCP_CHUNK_SIZE_MAX), 1, OCP_CHUNK_SIZE_MAX);
    ...
    auto ocpDevice = std::make_unique<OCPDevice::OCPRecoveryDevice>(
        ...
        static_cast<uint8_t>(memoryWindow.value_or(OCP_CMS_DEFAULT)),
