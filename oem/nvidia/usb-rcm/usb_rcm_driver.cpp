...
std::vector<PcqEntry> readProgressQueue(ReadReg readReg)
{
    ...
    uint32_t count = full ? queueSize
                          : (endIndex + queueSize - startIndex) % queueSize;
...
std::optional<uint8_t> recoveryBulkOutEndpoint(
    libusb_device* device, int recoveryInterface, uint8_t recoveryEndpoint)
{
                                       ...
                                       bool verbose)
{
    ...
    if (!blobPath.empty())
    {
        ...
        emptyDotBlob = blobData.size() == emptyDotBlobSize &&
