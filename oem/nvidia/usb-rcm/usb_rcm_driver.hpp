...
struct DeviceConfig
{
    ...
    // auto-discover it from the interface descriptor, which is correct for every
    // revision (the reference exposes it at 0x08, some parts at 0x0a). Set a
    // non-zero value in the USBRCMFirmware record only to pin a specific
    // endpoint number.
...
class RecoveryDriver
{
  public:
    ...
    // the ECID to decide whether a DOT blob is required and, if so, prepends the
    // per-CPU blob from dotBlobDir. `images` must already be in RCM flash order.
    RecoveryResult recover(const std::vector<Image>& images, bool verbose = false);
