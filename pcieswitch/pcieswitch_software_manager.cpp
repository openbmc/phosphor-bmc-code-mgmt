    ...
    const std::string& service, const std::string& path, SoftwareConfig& config)
{
    ...
    if (!chipName.has_value() || !busNo.has_value() || !address.has_value())
    {
    ...
    for (size_t i = 0; true; i++)
    {
        ...
        info("MuxOutput{NUM} name: {NAME}, value: {VAL}", "NUM",
             std::to_string(i), "NAME", gpioLines[i], "VAL", gpioValues[i]);
    }
    ...
    try
    {
        spiDevice = std::make_unique<SPIDevice>(
            ctx, chipName.value(), gpioLines, gpioValues, config, this);
    }
