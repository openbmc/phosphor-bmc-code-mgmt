...
#include <phosphor-logging/log.hpp> //TODO: REMOVE
...
using namespace phosphor::logging; //TODO: REMOVE
...
bool restoreFromFile(std::string versionId, uint8_t& priority)
{
    ...
    if (!devicePath.empty())
    {
        ...
        {
            // Grab the environment variable for this versionId. These variables
            // follow the format "versionId=priority\0"
            std::string var = envVars.substr(envVars.find(versionId));

            std::string::size_type posEqual = var.find('=');
            std::string::size_type posNull = var.find('\0');
            priority = atoi(var.substr(posEqual + 1,
                    posNull - posEqual).c_str());
