...
{
    init(extractPath, SIGNED_IMAGE_CONF_PATH);
}

SignatureConfig::SignatureConfig(const fs::path& extractPath,
                                 const fs::path& signedConfPath)
{
    init(extractPath, signedConfPath);
}
...
void SignatureConfig::init( const fs::path& extractPath,
                            const fs::path& signConfPath)
{
