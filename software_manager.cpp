...
using namespace phosphor::software::utils;
using namespace sdbusplus::error::xyz::openbmc_project::common;
using namespace sdbusplus::error::xyz::openbmc_project::software::update;

ApplyTimeIntf::RequestedApplyTimes
    Manager::convertApplyTimes(ApplyTimes applyTime)
{
    switch (applyTime)
    {
        case ApplyTimes::OnReset:
        {
            return ApplyTimeIntf::RequestedApplyTimes::OnReset;
        }
        case ApplyTimes::Immediate:
        default:
        {
...
auto Manager::processImage(sdbusplus::message::unix_fd image,
                           ApplyTimes applyTime, std::string id,
                           std::string objPath) -> sdbusplus::async::task<>
{
    debug("Processing image {FD}", "FD", static_cast<int>(image));
    ...
    if (!unTar(image, tmpDirPath.string()))
    {
    ...
    auto filePath = tmpDirPath.string();
    ...
    if (applyTime == ApplyTimes::Immediate || applyTime == ApplyTimes::OnReset)
    {
...
{
    debug("Starting update for image {FD}", "FD", static_cast<int>(image));
    ...
    itemUpdater->createActivationWithApplyTime(id, objPath,
                                               convertApplyTimes(applyTime));
...
int main()
{
    ...
    Manager softwareManager{ctx, path};
