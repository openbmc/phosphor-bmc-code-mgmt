    ...
    for (const auto& chipEnum : supportedSpiChips)
    {
        ...
        SPIFactory::instance().registerSPI(
            ...
            [chipEnum](sdbusplus::async::context& ctx, uint64_t ctrlIdx,
                       ...
                       SoftwareManager* parent) -> std::unique_ptr<SPIDevice> {
                switch (chipEnum)
                {
                    case spiChip::INTEL_HOST_BIOS:
                        return std::make_unique<BIOSDevice>(
                            ctx, ctrlIdx, devIdx, dryRun, names, values, config,
                            parent, flashLayoutFlat, flashToolFlashrom,
                            "/var/bios/", "host0_bios_version.txt");

                    case spiChip::HOST_BIOS:
                        return std::make_unique<BIOSDevice>(
                            ctx, ctrlIdx, devIdx, dryRun, names, values, config,
                            parent, flashLayoutFlat, flashToolFlashcp,
                            "/var/bios/", "host0_bios_version.txt");

                    case spiChip::INTEL_E810_NIC:
