...
// NOLINTEND
{
    ...
    const Software* oldCurrent = device->currentSoftware.get();

    device->currentSoftware->setVersion("vUnknown");
    ...
    assert(device->currentSoftware.get() == oldCurrent);

    // assert that the updated software is present
    assert(device->softwarePending.get() == updateSoftware);
