...
class FWUpdateEventsTest : public ::testing::Test
{
  public:
    ...
    auto testApplyFailedAssertDeassert() -> sdbusplus::async::task<void>
    {
        eventServer.expectedEvent = error_intf::ApplyFailed::errName;
        co_await events.generateApplyFailed(targetObjectPath, imageIdentifier,
                                            true);
...
{
    ctx.spawn(testApplyFailedAssertDeassert());
    ctx.run();
}
