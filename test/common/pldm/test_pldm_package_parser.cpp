    ...
    const std::shared_ptr<PackageParser>& packageParser)
{
    ...
    {
        error("fw device id records != 1, rejecting package, incompatible.");
        return "";
    }

    debug("package has {N} fw device id records", "N",
    ...
    for (FirmwareDeviceIDRecord& record : fwDeviceIdRecords)
    {
        ...
        debug("fw device id record has {N} descriptors", "N", desc.size());
...
TEST(PLDM_PACKAGE_PARSER, getCompatible)
{
    ...
    debug("compatible = {COMPATIBLE}", "COMPATIBLE", compatible);
...
TEST(PLDM_PACKAGE_PARSER, getComponentImage)
{
    ...
    debug("found {N} component images", "N", compImages.size());
    ...
    debug("component classification: {VALUE}", "VALUE", compClass);

    const uint16_t compIdentifier = std::get<1>(compImage);
    debug("component identifier: {VALUE}", "VALUE", compIdentifier);

    const uint32_t compLocationOffset = std::get<5>(compImage);
    debug("component location offset: {VALUE}", "VALUE", compLocationOffset);

    const uint32_t compSize = std::get<6>(compImage);
    debug("component size: {VALUE}", "VALUE", compSize);

    assert(compSize == component_image_size);

    std::string compVersion = std::get<7>(compImage);
    debug("component version: {VALUE}", "VALUE", compVersion);

    uint8_t* p = buf.get() + compLocationOffset;
    debug("first few bytes of component image: {1} {2} {3} {4}", "1", lg2::hex,
          p[0], "2", lg2::hex, p[1], "3", lg2::hex, p[2], "4", lg2::hex, p[3]);
...
TEST(PLDM_PACKAGE_PARSER, getFirmwareDeviceId)
{
    ...
    auto fwDeviceIdRecords = packageParser->getFwDeviceIDRecords();
    debug("found {N} fw device id records", "N", fwDeviceIdRecords.size());
...
TEST(PLDM_PACKAGE_PARSER, getIANA)
{
    ...
    if (desc.empty())
    {
        error("there is no descriptor for the applicable device");
        assert(false);
    }
    ...
    if (!desc.contains(1))
    {
        ...
        error("the package does not have an IANA type descriptor");
        assert(false);
    }
    ...
    if (data.size() != 4)
    {
        error("iana id should be 4 bytes");
        assert(false);
    }
    ...
    debug("iana = {IANA}", "IANA", lg2::hex, iana);
