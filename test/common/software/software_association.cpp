    ...
    std::unique_ptr<ExampleDevice>& device)
{
    ...
    auto optBusName = exampleUpdater.getBusName();

    EXPECT_TRUE(optBusName.has_value());

    if (!optBusName.has_value())
    {
        ctx.request_stop();
        co_return;
    }

    const std::string& busName = optBusName.value();
    ...
    std::unique_ptr<ExampleDevice>& device)
{
    ...
    device->softwareCurrent->createInventoryAssociation(true, exampleEndpoint);
    ...
    std::unique_ptr<ExampleDevice>& device)
{
    ...
    device->softwareCurrent->createInventoryAssociation(false, exampleEndpoint);
