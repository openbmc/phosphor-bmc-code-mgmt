    ...
    const std::string& objPathCurrentSoftware, const std::string& busName)
{
    ...
    try
    {
        ...
        EXPECT_EQ(std::get<0>(res[0]), "running");
...
{
    ctx.spawn(testSoftwareAssociationRunning(ctx, device,
                                             objPathCurrentSoftware, busName));
    ctx.run();
}
    ...
    const std::string& objPathCurrentSoftware, const std::string& busName)
{
    ...
    try
    {
        ...
        EXPECT_EQ(res.size(), 1);
        EXPECT_EQ(std::get<0>(res[0]), "activating");
        EXPECT_EQ(std::get<2>(res[0]), exampleEndpoint);
    }
    catch (std::exception& e)
    {
...
TEST_F(SoftwareAssocTest, TestSoftwareAssociationActivating)
{
    ...
    ctx.spawn(testSoftwareAssociationActivating(
        ctx, device, objPathCurrentSoftware, busName));
    ctx.run();
}
