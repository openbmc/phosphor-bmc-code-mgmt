...
class SignatureTest : public testing::Test
{
    protected:
        virtual void SetUp()
        {
            ...
            std::system("openssl rsa -in images/private.pem -outform PEM -pubout -out images/publickey");
            std::system("cp images/publickey conf/OpenBMC/publickey");
            std::system("openssl dgst -sha256 -sign images/private.pem -out images/image-kernel.sig images/image-kernel");
            std::system("openssl dgst -sha256 -sign images/private.pem -out images/MANIFEST.sig images/MANIFEST");
            std::system("openssl dgst -sha256 -sign images/private.pem -out images/image-rofs.sig images/image-rofs");
            std::system("openssl dgst -sha256 -sign images/private.pem -out images/image-rwfs.sig images/image-rwfs");
            std::system("openssl dgst -sha256 -sign images/private.pem -out images/image-u-boot.sig images/image-u-boot");
            std::system("openssl dgst -sha256 -sign images/private.pem -out images/publickey.sig images/publickey");
        }
        virtual void TearDown()
        {
            std::system("rm -rf ./conf/");
