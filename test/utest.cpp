...
protected:
  ...
  virtual void TearDown() { fs::remove_all(_directory); }
...
protected:
  ...
  virtual void SetUp() {
    ...
    std::string hashFile = _signedConfOpenBMCPath.string() + "/hashfunc";
    ...
    std::string ubootFile = _extractPath.string() + "/" + "image-u-boot";
    ...
    command("openssl rsa -in " + pkeyFile + " -outform PEM " + "-pubout -out " +
            pubkeyFile);
    ...
    command(opensslCmd + pkeyFile + " -out " + manifestFile + ".sig " +
            manifestFile);
    command(opensslCmd + pkeyFile + " -out " + rofsFile + ".sig " + rofsFile);
    command(opensslCmd + pkeyFile + " -out " + rwfsFile + ".sig " + rwfsFile);
    command(opensslCmd + pkeyFile + " -out " + ubootFile + ".sig " + ubootFile);
    command(opensslCmd + pkeyFile + " -out " + pubkeyFile + ".sig " +
            pubkeyFile);

    _signature = std::make_unique<Signature>(_extractPath, _signedConfPath);
  }
