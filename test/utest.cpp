...
TEST_F(KeyManagerTest, TestGetHashTypeFromSystem)
{
    keyManager->saveHashTypeToSystem();
    EXPECT_EQ(keyManager->getHashTypeFromSystem(), sha512);
}
...
TEST_F(KeyManagerTest, TestGetAllKeysAndHashFromSystem)
{
    ...
    AllKeysAndHash keysAndHashList = keyManager->getAllKeysAndHashFromSystem();
    for(auto key: keysAndHashList)
    {
        EXPECT_EQ(key.first, OpenBMC);
        KeyHashPair pair = key.second;
        EXPECT_EQ(pair.first, pubpath.string());
        EXPECT_EQ(pair.second, hashpath.string());
        break;
    }
}
