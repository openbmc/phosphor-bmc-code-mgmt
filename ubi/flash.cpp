...
{
fprintf(stderr, "UBI onStateChanges\n");
    uint32_t newStateID{};
    ...
    if (newStateUnit == roServiceFile && newStateResult == "done")
    {
        ...
        activationProgress->progress(
            activationProgress->progress() + 50);
    }
        ...
        newStateUnit == ubootVarsServiceFile)
    {
        ...
        else if ((rwVolumeCreated &&
                  roVolumeCreated) ||   // Volumes were created
                 (ubootEnvVarsUpdated)) // Environment variables were
                                               // updated
        {
            Activation::activation(
