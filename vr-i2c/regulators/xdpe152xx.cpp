...
*/
int XDPE152XX::fw_update(VR::Config* cfg, bool force)
{
    uint8_t remain = 0;
    this->check_image(cfg);
    this->programm(cfg, force);
...
int XDPE152XX::fw_parse_file(VR::Config& cfg, const uint8_t* image,
                             size_t image_size)
{
    ...
    std::string path = "/tmp/vr-image.bin";
    int image_fd = open(path.c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0644);

    if (image_fd < 0)
    {
        lg2::error("XDPE152XX::parse_file] Failed to open file: {PATH}", "PATH",
                   path);
        return -1;
    }

    const ssize_t bytesWritten = write(image_fd, image, image_size);

    if (bytesWritten < 0 || static_cast<size_t>(bytesWritten) != image_size)
    {
        lg2::error("[XDPE152XX::parse_file] Failed to write image to file");
        return -1;
    }
...
int XDPE152XX::fw_verify(VR::Config* cfg)
{
